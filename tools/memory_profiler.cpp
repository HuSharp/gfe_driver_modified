...
// internally malloc uses the last 3 bits of the allocated size as control bits
// : PREV_INUSE, IS_MAPPED, NON_MAIN_ARENA mask them to get the actual allocated
// size (including overhead).
static constexpr uint64_t MASK_MALLOC_SIZE = ~static_cast<uint64_t>((0x1 | 0x2 | 0x4));

// dlsym invokes calloc. We initially resolve these invocations through a small
// memory pool
void * gfe_dummy_calloc(size_t num, size_t size)
{
    ...
    { // overflow
        fprintf(
            stderr,
            "[memory profiler error] depleted the space in the initial "
            "memory pool\n");
        exit(EXIT_FAILURE);
    }
...
struct RecursionGuard
{
    ...
    ~RecursionGuard() { g_recursion = false; }
};
} // anonymous namespace
...
#define PREAMBLE(action)                                                           \
    ...
    {                                                                              \
        return action;                                                             \
    } /* avoid that the hooks being fired when the functions are invoked by      \
       glibc */ \
...
void * mmap(void * addr, size_t length, int prot, int flags, int fd, off_t offset)
{
    ...
    if (ret != MAP_FAILED)
    { // record the mapping
        // printf("mmap, return address: %p (%" PRIu64 "), length: %zu, prot: %d,
        // flags: %d, fd: %d, offset: %ld \n", ret, (uint64_t) ret, length, prot,
        // flags, fd, offset);
        handle_mmap(ret);
    }
...
int64_t gfe_compute_memory_footprint()
{
    ...
    while (fgets(buffer, sizeof(buffer), fp) != nullptr && g_next_mapping < g_num_memory_mappings)
    {
        ...
        if (address < g_memory_mappings[g_next_mapping])
            continue; // skip
        while (g_next_mapping < g_num_memory_mappings && address > g_memory_mappings[g_next_mapping])
        {
            fprintf(
                stderr,
                "[memory profiler error] address not matched: %" PRIu64 "\n",
                g_memory_mappings[g_next_mapping]);
            g_next_mapping++;
        }
        if (g_next_mapping < g_num_memory_mappings && address == g_memory_mappings[g_next_mapping])
        { // match
