...
#define LOG(msg)                                               \
    {                                                          \
        ...
        std::cout << msg << /* flush immediately */ std::endl; \
    }
 ...
 * - Initialise (only one time) the singleton instance through
 *Configuration::initialise(int argc, char* argv[])
...
class Configuration
{
    ...
    // The max weight that can be assigned by graph readers when parsing a non
    // weighted graph
...
public:
    ...
    // Coefficient for the surplus of updates to perform (noise) w.r.t. the
    // final graph  to load
    ...
    // The budget to complete a Graphalytics algorithm, in seconds (e.g. LCC
    // should terminate by get_timeout_graphalytics() seconds)
    ...
    // Get the expansion factor in the aging experiment for the edges in the
    // graph
    double get_ef_edges() const { return m_ef_edges; }

    // Get the expansion factor in the aging experiment for the vertices in the
    // graph
    ...
    // Get the cool-off period in the aging experiment. After the experiment
    // terminates, the driver waits for the given amount of seconds idle,
    // checking the amount of memory used. The goal is to detect the impact of
    // the garbage collector of the evaluated library in reducing the memory
    // footprint when no updates are being executed.
    ...
    // Forcedly stop the execution of the aging2 experiment if the process is
    // using more memory than this threshold, in bytes
    uint64_t get_aging_memfp_threshold() const { return m_aging_memfp_threshold; }

    // Whether to print to stdout the measurements observed for the memory
    // footprint
    bool get_aging_memfp_report() const { return m_aging_memfp_report; }

    // Whether to consider the physical memory or the virtual memory in the
    // memory footprint
    ...
    // Retrieve the handle to the database connection, where the final results
    // of the experiments are stored
    ::common::Database * db();
    ...
    // Remove the algorithms blacklisted by the user in the
    // GraphalyticsAlgorithms list
