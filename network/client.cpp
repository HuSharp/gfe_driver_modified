...
void Client::connect()
{
    if (m_connections[m_worker_id].m_fd >= 0)
    ...
    { // gethostbyname() is not thread safe, wrap it inside a global lock
        ...
        {
            ERROR(
                "Cannot resolve the address of the remote server `" << m_server_host << "': " << gai_strerror(h_errno));
        }
...
{
    if (worker_id >= max_num_connections)
        ERROR("Invalid worker_id: " << worker_id);
    if (m_connections[worker_id].m_fd == -1)
        return;
    close(m_connections[worker_id].m_fd);
    m_connections[worker_id].m_fd = -1;
    free(m_connections[worker_id].m_buffer_read);
    m_connections[worker_id].m_buffer_read = nullptr;
    free(m_connections[worker_id].m_buffer_write);
    m_connections[worker_id].m_buffer_write = nullptr;
}
...
void Client::request(RequestType type, Args... args)
{
    ...
    if (bytes_sent == -1)
        ERROR_ERRNO("send_request, connection error");
...
void Client::wait_response()
{
    ...
    if (message_sz > m_connections[m_worker_id].m_buffer_read_sz)
    { // realloc the buffer if it's not large enough
...
{
    if (thread_id < 0 || thread_id > max_num_connections)
...
bool Client::add_edge(graph::WeightedEdge e)
{
    ...
    {
        ERROR(
            "add_edge(" << e.source() << ", " << e.destination() << ", " << e.weight()
                        << "): operation not supported by the remote interface");
    }
    assert(response()->type() == ResponseType::OK);
    return response()->get<bool>(0);
}
...
bool Client::remove_edge(graph::Edge e)
{
    ...
    {
        ERROR(
            "delete_edge(" << e.source() << ", " << e.destination()
                           << "): operation not supported by the remote interface");
    }
    assert(response()->type() == ResponseType::OK);
    return response()->get<bool>(0);
...
bool Client::batch(const library::UpdateInterface::SingleUpdate * batch, uint64_t batch_sz, bool force)
{
    ...
    // this is actually more expensive than memcpy into the buffer and doing one
    // send();
    //    // send the header
    //    assert(m_worker_id >= 0 && m_worker_id < max_num_connections && "Invalid
    //    worker id"); uint32_t* buffer_write =
    //    reinterpret_cast<uint32_t*>(m_connections[m_worker_id].m_buffer_write);
    //    buffer_write[0] = header_sz;
    //    buffer_write[1] = (uint32_t) RequestType::BATCH_PLAIN;
    //    ssize_t bytes_sent = send(m_connections[m_worker_id].m_fd, buffer_write,
    //    header_sz, /* flags */ 0); if(bytes_sent == -1)
    //    ERROR_ERRNO("send_request, header, connection error"); assert(bytes_sent
    //    == header_sz && "Message not fully sent");
    //
    //    // send the body
    //    bytes_sent = send(m_connections[m_worker_id].m_fd, (void*) batch,
    //    body_sz, /* flags */ 0); if(bytes_sent == -1) ERROR_ERRNO("send_request,
    //    body, connection error"); assert(bytes_sent == body_sz && "Message not
    //    fully sent");
    ...
    if (response()->type() == ResponseType::TIMEOUT)
    {
        assert(
            force == true
            && "Only with the flag force == true, the interface "
               "is allowed to timeout");
...
{
    if (dump2file != nullptr && dump2file[0] == '\0')
        dump2file = nullptr;
    ...
    {
        ERROR(
            "bfs(" << source_vertex_id << ", \"" << dump2file
                   << "\"): operation not supported by the remote interface");
    }
    else if (response()->type() == ResponseType::ERROR)
    {
...
void Client::pagerank(uint64_t num_iterations, double damping_factor, const char * dump2file)
{
    ...
    {
        ERROR(
            "pagerank(" << num_iterations << ", " << damping_factor << ", \"" << dump2file
                        << "\"): operation not supported by the remote interface");
    }
    else if (response()->type() == ResponseType::TIMEOUT)
    {
...
void Client::cdlp(uint64_t max_iterations, const char * dump2file)
{
    ...
    case ResponseType::NOT_SUPPORTED:
        ERROR(
            "cdlp(" << max_iterations << ", \"" << dump2file << "\"): operation not supported by the remote interface");
        break;
...
void Client::sssp(uint64_t source_vertex_id, const char * dump2file)
{
    ...
    case ResponseType::NOT_SUPPORTED:
        ERROR(
            "sssp(" << source_vertex_id << ", \"" << dump2file
                    << "\"): operation not supported by the remote interface");
        break;
    case ResponseType::TIMEOUT:
        TIMEOUT_ERROR
