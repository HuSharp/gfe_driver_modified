...
static void signal_handler_install()
{
    ...
    if (rc != 0)
        ERROR_ERRNO("sigaction, sigterm [rc: " << rc << "]");
    rc = sigaction(SIGINT, &sa, &g_sigaction_interrupt);
    if (rc != 0)
        ERROR_ERRNO("sigaction, sigint");
}
...
static void signal_handler_uninstall()
{
    ...
    if (rc != 0)
    {
        cerr << "ERROR: signal_handler_uninstall, sigaction, sigterm, rc: " << rc << endl;
    }
    rc = sigaction(SIGINT, &g_sigaction_interrupt, nullptr);
    if (rc != 0)
    {
        cerr << "ERROR: signal_handler_uninstall, sigaction, sigint, rc: " << rc << endl;
    }
}
...
Server::Server(shared_ptr<library::Interface> interface, int port)
    : m_interface(interface)
    , m_port(port)
{
    int fd = socket(AF_INET, SOCK_STREAM, 0);
    if (fd < 0)
        ERROR_ERRNO("Cannot initialise the socket");
    m_server_fd = fd;
    ...
    if (rc != 0)
        ERROR_ERRNO("Cannot bind the server socket to port: " << m_port);

    // be ready to accept connections
    rc = listen(m_server_fd, SOMAXCONN);
    if (rc != 0)
        ERROR_ERRNO("Error while attempting to make the socket ready");
}
...
void Server::stop()
{ // Stop once there are no more connections active
    m_server_stop = true;
}
...
void Server::handle_signals()
{
    ...
    {
        ERROR(
            "A signal handler is already installed for another instance of this "
            "class: "
            << g_server_instance);
    }
...
void Server::main_loop()
{
    ...
    while (!m_server_stop)
    {
        ...
        fd_set dummy0;
        FD_ZERO(&dummy0); // because of __restrict we need both dummy0 and dummy1 to
        ...
        if (ready < 0)
        {
            if (m_server_stop)
            {
                LOG("[server] Call to select() failed, server requested to "
                    "terminate...");
                m_server_stop = true;
                continue;
            }
        ...
        { // timeout, check the flag m_server_stop
            if (m_terminate_on_last_connection && m_num_active_connections == 0)
                m_server_stop = true; // done
            continue;
        }

        // there is only one fd that can be set, no need to check the bitmask from
        // select
        struct sockaddr_in address;
        ...
        //        LOG("[server] Connection received from: " <<
 ...
 *                                                                           *
 *****************************************************************************/
Server::ConnectionHandler::ConnectionHandler(Server * instance, int fd)
    : m_instance(instance)
    , m_fd(fd)
{
...
void Server::ConnectionHandler::execute()
{
    ...
    COUT_DEBUG(
        "[server] [thread " << thread_id << "] Connected with " << remote_host << ":" << remote_port
                            << ", num active connections: " << num_active_connections);
    ...
    while (!m_terminate)
    {
        ...
        {
            LOG("[server] [thread " << thread_id
                                    << "] Connection closed by the remote end "
                                       "without sending a TERMINATE_WORKER message");
            m_terminate = true;
            break;
        }
        ...
        if (message_sz > (int64_t)m_buffer_read_sz)
        { // realloc the buffer if it is too small
            m_buffer_read_sz = pow(2, ceil(log2(message_sz))); // next power of 2
            LOG("[server] [thread " << thread_id << "] Reallocate the internal read buffer to " << m_buffer_read_sz
                                    << " bytes");
        ...
        while (num_bytes_read < message_sz)
        {
            ...
            if (recv_bytes == -1)
                ERROR_ERRNO(
                    "recv, only able to read " << num_bytes_read << " out of " << num_bytes_read
                                               << " bytes, then the connection was interrupted?");
            num_bytes_read += recv_bytes;
        }
    ...
    COUT_DEBUG(
        "[server] [thread " << thread_id << "] Disconnected with " << remote_host << ":" << remote_port
                            << ", remaining active connections: " << num_active_connections);
...
void Server::ConnectionHandler::handle_request()
{
    try
    {
        ...
        case RequestType::BATCH_PLAIN_FORCE_YES:
        {
            ...
            else
            {
                try
                {
                    ...
                    const library::UpdateInterface::SingleUpdate * batch
                        = reinterpret_cast<const library::UpdateInterface::SingleUpdate *>(request()->buffer());
        ...
        break;
            ////     Only to measure the overhead of the network connections for
            /// updates
            //    case RequestType::ADD_VERTEX:
            //    case RequestType::REMOVE_VERTEX:
            //    case RequestType::ADD_EDGE:
...
void Server::ConnectionHandler::response(ResponseType type, Args... args)
{
    ...
    assert(
        /* message_sz */ (*(reinterpret_cast<uint32_t *>(m_buffer_write))) < m_buffer_write_sz
...
void Server::ConnectionHandler::send_data(const char * buffer, uint32_t buffer_sz)
{
    ...
    if (bytes_sent == -1)
        ERROR_ERRNO("send_response, connection error");
