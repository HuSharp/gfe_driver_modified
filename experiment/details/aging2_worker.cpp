...
#define COUT_DEBUG_FORCE(msg)                                                              \
    {                                                                                      \
        ...
        cout << "[Aging2Worker::" << __FUNCTION__ << "] [" << concurrency::get_thread_id() \
             << ", worker_id: " << m_worker_id << "] " << msg << endl;                     \
    }
#if defined(DEBUG)
...
Aging2Worker::Aging2Worker(Aging2Master & master, int worker_id)
    : m_master(master)
...
{
    set_task_async(
        TaskOp::SET_ARRAY_LATENCIES,
        array_insertions,
        /* hack */ reinterpret_cast<uint64_t>(array_deletions));
}
...
void Aging2Worker::set_task_async(TaskOp type, uint64_t * payload, uint64_t payload_sz)
{
    { // restrict the scope
        ...
        assert(m_task.m_type == TaskOp::IDLE && "Service busy");
        if (m_task.m_type != TaskOp::IDLE)
            ERROR("Background thread already busy performing another operation");
...
bool Aging2Worker::wait(const chrono::time_point<chrono::steady_clock> & tp)
{
    ...
    unique_lock<mutex> lock(m_mutex);
    while (m_task.m_type != TaskOp::IDLE &&
           /* timeout ? */ !((tp <= now) || (tp - now) < 1ms))
    {
...
void Aging2Worker::main_thread()
{
    ...
    do
    {
        ...
        case TaskOp::START:
            assert(
                0
                && "This operation is reserved only for starting the service, "
                   "it should not occur anymore at this point");
            break;
        case TaskOp::STOP:
            terminate = true;
            break;
...
void Aging2Worker::main_execute_updates()
{
    ...
    // reports_per_ops only affects how often a report is saved in the db, not the
    ...
    {
        // if we're release the driver's memory, always fetch the first. Otherwise
        // follow the index.
        ...
        for (uint64_t j = 0; j < num_loops; j++)
        {
            ...
            // report how long it took to perform 1x, 2x, ... updates w.r.t. to the
            ...
            {
                if (m_master.m_last_time_reported.compare_exchange_strong(
                        /* updates lastset_coeff */ lastset_coeff,
                        aging_coeff))
                {
                    uint64_t duration = chrono::duration_cast<chrono::microseconds>(
                                            chrono::steady_clock::now() - m_master.m_time_start)
                                            .count();
        ...
        if (release_memory)
        {
            ...
            {
                m_updates_mem_usage
                    -= m_updates[0]->size() * sizeof(gfe::graph::WeightedEdge); // update the memory footprint of
                // this worker
            }
            else
            { // virtual memory
...
void Aging2Worker::main_load_edges(uint64_t * edges, uint64_t num_edges)
{
    if (m_updates.empty())
    {
        m_updates.append(new vector<graph::WeightedEdge>());
    }
    ...
    for (uint64_t i = 0; i < num_edges; i++)
    {
        if (static_cast<int>((sources[i] + destinations[i]) % modulo) == m_worker_id)
        {
            ...
            if (weight == 0.0)
            {
                ...
                if (weight == 0.0)
                    weight = m_master.parameters().m_max_weight; // in (0, max_weight]
            }
...
{
    if (!m_master.is_directed() && m_uniform(m_random) < 0.5)
        edge.swap_src_dst(); // noise
...
void Aging2Worker::graph_remove_edge(graph::Edge edge, bool force)
{
    ...
    { // measure the latency of the deletion
        chrono::steady_clock::time_point t0, t1;
