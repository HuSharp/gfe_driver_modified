...
#define COUT_DEBUG_FORCE(msg)                                                                                      \
    {                                                                                                              \
        ...
        cout << "[Aging2Master::" << __FUNCTION__ << "] [" << concurrency::get_thread_id() << "] " << msg << endl; \
    }
#if defined(DEBUG)
    ...
    , m_results(parameters)
{
    ...
    m_reported_times = new uint64_t[static_cast<uint64_t>(
        m_parameters.m_num_reports_per_operations
            * ::ceil(static_cast<double>(num_operations_total()) / num_edges_final_graph())
        + 1)]();
...
void Aging2Master::load_edges()
{
    ...
    while (num_edges > 0)
    {
        ...
        for (auto w : m_workers)
            w->load_edges(array1, num_edges);
        if (m_results.m_random_vertex_id == 0)
        {
            set_random_vertex_id(array1, num_edges);
        }
...
void Aging2Master::do_run_experiment()
{
    ...
    BuildThread build_service{
        parameters().m_library,
        static_cast<int>(parameters().m_num_threads) + 2,
...
void Aging2Master::remove_vertices()
{
    ...
    for (auto w : m_workers)
        w->remove_vertices(vertices, num_vertices);
    for (auto w : m_workers)
        w->wait();
    m_parameters.m_library->build();

    LOG("[Aging2] Number of extra vertices: "
        << m_results.m_num_artificial_vertices
        << ", "
           "expansion factor: "
        << static_cast<double>(m_results.m_num_artificial_vertices + m_results.m_num_vertices_final_graph)
    ...
    do
    {
        ...
        if (!done)
        {
                ...
                && (/* first tick */ (m_results.m_progress.size() == 1) || tp - last_memory_footprint_recording >= 10s))
            {
                ...
                uint64_t memfp_process = measure_physical_memory
                    ? common::get_memory_footprint()
                    : max<int64_t>(utility::MemoryUsage::memory_footprint(), 0);
                uint64_t memfp_driver = memory_footprint();
                Aging2Result::MemoryFootprint memfp{
                    tick,
                    memfp_process,
                    memfp_driver,
                    /* cool off ? */ false};
                ...
                if (m_results.m_progress.size() > 1)
                {
                    last_memory_footprint_recording = tp;
                } // beyond the first tick
...
{
    if (m_parameters.m_cooloff.count() == 0)
    ...
    do
    {
        ...
        uint64_t memfp_process = measure_physical_memory ? common::get_memory_footprint()
                                                         : max<int64_t>(utility::MemoryUsage::memory_footprint(), 0);
        uint64_t memfp_driver = memory_footprint();
        Aging2Result::MemoryFootprint memfp{
            static_cast<uint64_t>(next_tick.count()),
            memfp_process,
            memfp_driver,
            /* cool off ? */ true};
        if (measure_memfp)
        ...
        if (report_memfp)
        {
            LOG("Memory footprint (cool-off) after " << DurationQuantity((uint64_t)next_tick.count()) << ": "
                                                     << ComputerQuantity(memfp_process - memfp_driver, true));
        }
        next_tick += 1s;
...
void Aging2Master::store_results()
{
    ...
    if (parameters().m_measure_latency)
    {
        ...
        m_results.m_latency_stats[0]
            = LatencyStatistics::compute_statistics(m_latencies, m_latencies_num_insertions); // insertions
        m_results.m_latency_stats[1] = LatencyStatistics::compute_statistics(
            m_latencies + m_latencies_num_insertions,
            m_latencies_num_deletions); // deletions
        m_results.m_latency_stats[2] = LatencyStatistics::compute_statistics(
            m_latencies,
            m_latencies_num_insertions + m_latencies_num_deletions); // both insertions & deletions
        ...
        LOG("[Aging2] Average latency of updates: " << DurationQuantity(m_results.m_latency_stats[2].mean())
                                                    << ", 99th percentile: "
                                                    << DurationQuantity(m_results.m_latency_stats[2].percentile99()));
...
void Aging2Master::log_num_vtx_edges()
{
    ...
    if (m_results.m_num_vertices_load == m_results.m_num_vertices_final_graph)
    {
        cout << "yes";
    }
    else
    ...
    if (m_results.m_num_edges_load == m_results.m_num_edges_final_graph)
    {
        cout << "yes";
    }
    else
...
uint64_t Aging2Master::memory_footprint() const
{
    ...
    for (auto & w : m_workers)
    {
        result += w->memory_footprint();
    }

    // size of the internal vectors
    if (parameters().m_memfp_physical)
    { // physical memory
        // result += sizeof(uint64_t) * static_cast<uint64_t>(
        // m_parameters.m_num_reports_per_operations * ::ceil(
        // static_cast<double>(num_operations_total())/num_edges_final_graph()) + 1
        // );
