...
Aging2Result::Aging2Result(const Aging2Experiment & parameters)
    : m_num_threads(parameters.m_num_threads)
    , m_worker_granularity(parameters.m_worker_granularity)
{}
...
void Aging2Result::save(common::Database * handle)
{
    ...
    if (handle == nullptr)
        INVALID_ARGUMENT("The handle to the database is a nullptr");
