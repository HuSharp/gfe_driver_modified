...
InsertOnly::InsertOnly(
    ...
    std::shared_ptr<gfe::graph::WeightedEdgeStream> graph,
    int64_t num_threads)
...
{
    if (m_num_threads == 0)
        ERROR("Invalid number of threads: " << m_num_threads);
}

void InsertOnly::set_scheduler_granularity(uint64_t granularity)
{
    if (granularity == 0)
        INVALID_ARGUMENT("The granularity of a chunk size cannot be zero");
...
static void run_sequential(
    library::UpdateInterface * interface,
    graph::WeightedEdgeStream * graph,
    uint64_t start,
    uint64_t end)
{
...
void InsertOnly::execute_round_robin()
{
    ...
    for (int64_t i = 0; i < m_num_threads; i++)
    {
        threads.emplace_back(
            [this, &start_chunk_next](int thread_id) {
...
{
    // re-adjust the scheduler granularity if there are too few insertions to
    // perform
    if (m_stream->num_edges() / m_num_threads < m_scheduler_granularity)
    {
        m_scheduler_granularity = m_stream->num_edges() / m_num_threads;
        if (m_scheduler_granularity == 0)
            m_scheduler_granularity = 1; // corner case
        LOG("InsertOnly: reset the scheduler granularity to " << m_scheduler_granularity
                                                              << " edge insertions per thread");
    }
    ...
    LOG("Edge stream size: " << m_stream->num_edges() << ", num edges stored in the graph: " << m_interface->num_edges()
                             << ", match: " << (m_stream->num_edges() == m_interface->num_edges() ? "yes" : "no"));
...
void InsertOnly::save()
{
    ...
    db.add("scheduler_granularity",
           m_scheduler_granularity); // the number of insertions performed by each
    // thread
    ...
    // version 20191125: build thread, build frequency taken into account,
    // scheduler set to round_robin, removed batch updates version 20191210:
    // difference between num_build_invocations (explicit invocations to #build())
    // and num_snapshots_created (actual number of deltas created by the impl)
    // version 20200625: rely on #add_edge_v2 to implicitly create the vertices.
    // This should alleviate the footprint of the driver for non scalable
    // implementations
