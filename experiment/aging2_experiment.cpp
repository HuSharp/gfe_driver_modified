...
#define COUT_DEBUG_FORCE(msg)                                                                                          \
    {                                                                                                                  \
        ...
        cout << "[Aging2Experiment::" << __FUNCTION__ << "] [" << concurrency::get_thread_id() << "] " << msg << endl; \
    }
#if defined(DEBUG)
...
void Aging2Experiment::set_max_weight(double value)
{
    if (value <= 0)
    {
        INVALID_ARGUMENT("value <= 0: " << value);
    }
    m_max_weight = value;
}
...
void Aging2Experiment::set_parallelism_degree(uint64_t num_threads)
{
    if (num_threads < 1)
    {
        INVALID_ARGUMENT("num_threads < 1: " << num_threads);
    }
...
void Aging2Experiment::set_num_reports_per_ops(uint64_t value)
{
    if (value <= 0)
    {
        INVALID_ARGUMENT("The value specified must be >= 1, given: " << value);
    }
...
void Aging2Experiment::set_worker_granularity(uint64_t value)
{
    if (value < 1)
    {
        INVALID_ARGUMENT("value < 1: " << value);
    }
...
{
    if (m_library.get() == nullptr)
        ERROR("Library not set. Use #set_library to set it.");
    if (m_path_log.empty())
        ERROR("Path to the log file not set. Use #set_log to set it.")

    m_master = new details::Aging2Master(*this);
    auto result = m_master->execute();

    // Master should be deleted here to ensure the same thread that called the
    // constructor it also calls the destructor So, the on_thread_init matches the
    // on_thread_destroy.
    delete m_master;
    m_master = nullptr;
    return result;
