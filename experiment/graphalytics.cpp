...
#define COUT_DEBUG_FORCE(msg)                                                        \
    {                                                                                \
        std::cout << "[Graphalytics @ " << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
...
GraphalyticsAlgorithms::GraphalyticsAlgorithms(const string & path)
{
    ...
    {
        algorithm_name.erase(
            std::remove_if(begin(algorithm_name), end(algorithm_name), ::isspace),
            end(algorithm_name));
...
ostream & operator<<(std::ostream & out, const GraphalyticsAlgorithms & props)
{
    ...
    {
        out << " PageRank df: " << props.pagerank.m_damping_factor
            << ", num_iterations: " << props.pagerank.m_num_iterations << ";";
    }
 ...
 *                                                                           *
 ****************************************************************************/
GraphalyticsSequential::GraphalyticsSequential(
    std::shared_ptr<gfe::library::GraphalyticsInterface> interface,
    uint64_t num_repetitions,
    const GraphalyticsAlgorithms & properties)
...
std::chrono::microseconds GraphalyticsSequential::execute()
{
    ...
    for (uint64_t i = 0; i < m_num_repetitions; i++)
    {
        ...
        {
            LOG("Execution " << (i + 1) << "/" << m_num_repetitions
                             << ": BFS from source vertex: " << m_properties.bfs.m_source_vertex);
        ...
        {
            LOG("Execution " << (i + 1) << "/" << m_num_repetitions
                             << ": CDLP, max_iterations: " << m_properties.cdlp.m_max_iterations);
        ...
        {
            LOG("Execution " << (i + 1) << "/" << m_num_repetitions
                             << ": PageRank, damping factor: " << m_properties.pagerank.m_damping_factor
                             << ", num_iterations: " << m_properties.pagerank.m_num_iterations);
            ...
            try
            {
                t_local.start();
                interface->pagerank(
                    m_properties.pagerank.m_num_iterations,
                    m_properties.pagerank.m_damping_factor,
                    path_result);
                t_local.stop();
        ...
        {
            LOG("Execution " << (i + 1) << "/" << m_num_repetitions
                             << ": SSSP, source: " << m_properties.sssp.m_source_vertex);
...
void GraphalyticsSequential::report(bool save_in_db)
{
    ...
    if (!m_validate_results.empty())
    {
        ...
        {
            cout << ">> All executions succeeded the validation (" << m_validate_results.size() << " validation runs)"
                 << endl;
        }
        else
        {
            cout << ">> Executions that failed the validation: " << num_validation_errors << " out of "
                 << m_validate_results.size() << " validation runs" << endl;
        }
    }
}
...
void GraphalyticsSequential::set_validate_output(const std::string & path_property_file)
{
    ...
    { // generate the temporary folder
        stringstream ss;
        ss << P_tmpdir << "/"
           << "graphalytics.validate_output.XXXXXX";
        string dynpath = ss.str();
        constexpr uint64_t buffer_sz = 1024;
        char buffer[buffer_sz];
        if (buffer_sz < dynpath.size() + 1 /* \0 */)
            ERROR("Cannot generate the temporary path, the path name is too long");
        strcpy(buffer, dynpath.c_str());
        char * result = mkdtemp(buffer);
        if (result == nullptr)
        {
            ERROR("Cannot create the temporary path, mkdtemp error");
        }
...
void GraphalyticsSequential::set_validate_remap_vertices(const std::string & path_property_file)
{
    ...
    {
        ERROR(
            "The number of vertices in the two files `" << m_validate_path_expected << "' and `" << path_results
                                                        << "' do not match: " << expected_num_vertices
                                                        << " != " << results_num_vertices)
    }

    // the vertices in the results file must have been remapped following the same
    // sorted order of the expected file
    bool vtx_stable_map = reader_results.get_property("meta.stable-map") == "true";
    if (!vtx_stable_map)
    {
        ERROR(
            "[Validation] We cannot compare `" << m_validate_path_expected << "' and `" << path_results
                                               << "' because the vertices have "
                                                  "not been mapped following the same sorted order of the expected "
                                                  "input graph. The property `meta.stable-map' is not set or it is "
                                                  "false "
                                                  "in the file: `"
                                               << path_results << "'.")
    }
