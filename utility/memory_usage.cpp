...
// External variable, defined by POSIX, with the array of the env vars of the
// program
extern char ** environ;

// Default name of the library to preload. It can be overloaded with the env.
// var. GFE_MEMORY_PROFILER
...
static string path_memory_profiler()
{
    ...
    {
        library_name = DEFAULT_LIBRARY_NAME_MEMORY_PROFILER;
    }
...
bool MemoryUsage::is_initialised()
{
    ...
    {
        fn_compute_memory_footprint = reinterpret_cast<decltype(fn_compute_memory_footprint)>(
            dlsym(RTLD_DEFAULT, "gfe_compute_memory_footprint"));
    }
...
{
    if (fn_compute_memory_footprint != nullptr)
    { // trampoline to the actual implementation
...
uint64_t MemoryUsage::get_allocated_space(const void * pointer)
{
    if (pointer == nullptr)
        return 0;
    return reinterpret_cast<const uint64_t *>(pointer)[-1] &
        /* glibc flags */ ~7ull;
}
