...
#define COUT_DEBUG_FORCE(msg)                                                    \
    {                                                                            \
        ...
        std::cout << "[StingerDV::" << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
...
StingerDV::StingerDV(bool directed)
    : m_directed(directed)
{
...
{
    return stinger_num_active_vertices(STINGER); // stinger_num_active_vertices only checks for vertices that
...
{
    if (vertex_id >= STINGER->max_nv)
        ERROR("Invalid vertex ID: " << vertex_id << ", max vertex id allowed: " << STINGER->max_nv);
    return true; // lie
}
...
void StingerDV::cdlp(uint64_t max_iterations, const char * dump2file)
{
    ...
    auto ptr_labels0 = make_unique<int64_t[]>(N);
    int64_t * labels0 = ptr_labels0.get();
    auto ptr_labels1 = make_unique<int64_t[]>(N);
...
void StingerDV::lcc(const char * dump2file)
{
    ...
    common::Timer timer;
    timer.start();
    //    uint64_t N = stinger_max_nv(STINGER); // assumes static vertices, this
    //    is 4G as currently set in the config
    uint64_t N = stinger_max_active_vertex(STINGER) + 1;
    unique_ptr<uint64_t[]> ptr_result{new uint64_t[N]};
    uint64_t * __restrict result = ptr_result.get();
...
#pragma omp parallel for
    for (uint64_t v = 0; v < N; v++)
    {
        // timeout check
        if (tcheck->is_timeout())
            continue; // do not do any additional processing

        double coeff = 0.0; // by definition, if the vertex has less than two
            // neighbours, its clustering coefficient is zero
...
uint64_t StingerDV::lcc_count_triangles(int64_t vertex_id)
{
    ...
    stinger_gather_neighbors(
        STINGER,
        vertex_id,
        &returned_degree,
        neighbours,
        /* optional fields weights, timestamps 0 and 1, type */ nullptr,
        nullptr,
        nullptr,
        nullptr,
        degree);
    ...
    // consider both outgoing and incoming edges. Stinger ensures that a neighbour
    // in a directed graph that has both an incoming and outgoing edge is visited
    // only once
    STINGER_FORALL_EDGES_OF_VTX_BEGIN(STINGER, vertex_id)
    {
        if (STINGER_EDGE_DEST != vertex_id)
        { // ignore self loops
...
uint64_t StingerDV::lcc_count_intersections(
    int64_t vertex1,
    int64_t vertex2,
    int64_t * vertex1_neighbours,
    int64_t vertex1_neighbours_sz)
{
    assert(
        vertex1_neighbours_sz > 0
        && "If this vertex1 doesn't have any neighbours, then vertex2 cannot be "
           "a neighbour itself");
    ...
    /*
   * According to the Graphalytics spec. v1.0 of this algorithm, for both
   * undirected & directed graphs, we only account the outgoing edges at this
   * step, that is for the connection vertex2 -> vertex3. Whereas for the link
   * vertex1 <-> vertex2 we need to consider both incoming & outgoing edges for
   * directed graphs.
   */
...
void StingerDV::pagerank(uint64_t num_iterations, double damping_factor, const char * dump2file)
{
    ...
    auto ptr_rank0 = make_unique<double[]>(num_registered_vertices);
    double * rank0t = ptr_rank0.get();
    auto ptr_rank1 = make_unique<double[]>(num_registered_vertices);
    double * rank1t = ptr_rank1.get();

// init
#pragma omp parallel for
    for (uint64_t v = 0; v < num_registered_vertices; v++)
    {
        if (stinger_vtype_get(STINGER, v) == 1)
            continue; // vertex marked for deletion
...
#pragma omp parallel for reduction(+ : pr_constant)
        for (uint64_t v = 0; v < num_registered_vertices; v++)
        {
            ...
            if (stinger_outdegree(STINGER, v) == 0)
            {
                pr_constant += rank0[v];
            }

            // compute the score for the current iteration looking at the incoming
            // edges
            STINGER_FORALL_IN_EDGES_OF_VTX_BEGIN(STINGER, v)
            {
                ...
                if (outdegree > 0)
                {
                    rank1[v] += rank0[STINGER_EDGE_DEST] / outdegree;
                }
            }
            STINGER_FORALL_IN_EDGES_OF_VTX_END();
        }
...
#pragma omp parallel for
        for (uint64_t v = 0; v < num_registered_vertices; v++)
        {
            rank1[v] = (1.0 - damping_factor) / num_active_vertices
                + (rank1[v] + (pr_constant / num_active_vertices)) * damping_factor;
        }
...
// this is a simple comparison function for the queue that sorts based on the
// cost to reach a vertex
...
static std::vector<double> a_star(
    stinger_t * S,
    int64_t NV,
    ...
    chrono::seconds timeout)
{
    ...
    {
        cost_so_far[v] = std::numeric_limits<double>::max(); // initialize all the
            // values to + infinity
    }
    ...
    while (!frontier.empty())
    {
        ...
        STINGER_FORALL_OUT_EDGES_OF_VTX_BEGIN(S, current.vertex)
        {
            ...
            if (new_cost < cost_so_far[STINGER_EDGE_DEST]
                || cost_so_far[STINGER_EDGE_DEST] == std::numeric_limits<double>::max())
            {
...
void StingerDV::bfs(uint64_t source_vertex_id, const char * dump2file)
{
    auto result = a_star(
        STINGER,
        stinger_max_active_vertex(STINGER) + 1,
        source_vertex_id,
        /* all vertices */ -1,
        /* ignore weights */ true,
        chrono::seconds(m_timeout));
...
void StingerDV::dump_ostream(ostream & out) const
{
    ...
    {
        out << "[" << vertex_id << "] degree in/out/total: " << stinger_indegree_get(graph, vertex_id) << "/"
            << stinger_outdegree_get(graph, vertex_id) << "/" << stinger_degree_get(graph, vertex_id);
