...
{
    SortledtonDriver * non_const_this = const_cast<SortledtonDriver *>(this);
    SnapshotTransaction tx = non_const_this->tm.getSnapshotTransaction(
        ds,
...
bool SortledtonDriver::add_edge(gfe::graph::WeightedEdge e)
{
    ...
    // Even in the undirected case, we need to check only for the existence of one
    // edge direction to ensure consistency.
...
Worker::Worker(TopologyInterface & ds, Master * master)
    : ds(ds)
    , m_master(master)
{
