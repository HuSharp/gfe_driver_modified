...
{
SortledtonDriverV2::SortledtonDriverV2(bool is_graph_directed, int block_size)
...
{
    return tx.has_edge(
        sortledton::edge_t{static_cast<sortledton::dst_t>(source), static_cast<sortledton::dst_t>(destination)});
}
...
/**
 * Impose a timeout on each graph computation. A computation that does not
 * terminate by the given seconds will raise a TimeoutError.
 */
 ...
 * @return true if the vertex has been inserted, false otherwise (that is, the
...
/**
 * Remove the mapping for a given vertex. The actual internal vertex is not
...
bool SortledtonDriverV2::add_edge(gfe::graph::WeightedEdge e)
{
    ...
    sortledton::edge_t internal_edge{
        static_cast<sortledton::dst_t>(e.source()),
        static_cast<sortledton::dst_t>(e.destination())};

    sortledton::edge_t opposite{
        static_cast<sortledton::dst_t>(e.destination()),
        static_cast<sortledton::dst_t>(e.source())};
    ...
    //      // Even in the undirected case, we need to check only for the
    //      existence of one edge direction to ensure consistency.
    ...
    //      bool inserted = true;
    //      try {
    //        tx.insert_edge(internal_edge, (char *) &e.m_weight,
    //        sizeof(e.m_weight)); tx.insert_edge({internal_edge.dst,
    //        internal_edge.src}, (char *) &e.m_weight, sizeof(e.m_weight));
    //        inserted &= tx.execute();
...
bool SortledtonDriverV2::add_edge_v2(gfe::graph::WeightedEdge e)
{
    ...
    //        bool exists_reverse = tx.has_edge({internal_edge.dst,
    //        internal_edge.src}); if (exists != exists_reverse) {
    ...
    //
    //      tx.insert_edge({internal_edge.dst, internal_edge.src}, (char *)
    //      &e.m_weight, sizeof(e.m_weight)); tx.insert_edge(internal_edge, (char
    //      *) &e.m_weight, sizeof(e.m_weight));
    //
    //      bool inserted = true;
    //      inserted &= tx.execute();
    //
    ...
    //      bool exists_reverse = tx.get_weight({internal_edge.dst,
    //      internal_edge.src}, (char*) &out_reverse); if (!exists) {
    ...
    //      }
    //      if (out != out_reverse) {
    //        cout << "Edge sites have unequal weight: " << out << " " <<
    //        out_reverse << endl; cout << "This was an insertion: " << insertion
    //        << endl; cout << "In neighbourhood: " <<
    //        tx.neighbourhood_size(internal_edge.src) << " " <<
...
bool SortledtonDriverV2::remove_edge(gfe::graph::Edge e)
{
    ...
    //      auto tx = *tx_o;
    //
    //      edge_t internal_edge{static_cast<dst_t>(e.source()),
    //      static_cast<dst_t>(e.destination())};
    //
...
static vector<pair<uint64_t, uint>> translate_bfs(
    sortledton::storage::GraphStorageForwarder & tx,
...
bool Master::next_task(uint64_t * output_vtx_start /* inclusive */, uint64_t * output_vtx_end /* exclusive */)
{
...
Worker::Worker(sortledton::storage::GraphStorageForwarder & ds, Master * master)
    : ds(ds)
    , m_master(master)
{
