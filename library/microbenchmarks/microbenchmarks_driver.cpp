...
bool MicroBenchmarksDriver::add_vertex(uint64_t vertex_id)
{
    ...
    { // This does not seem to block until a is released.
        // It seems to continue with the information that
        // another thread is !currently! inserting this key.
        return false;
    }
    else
    {
...
{
    auto clusters = Gapbs::cdlp(
        *ds,
        [this](uint64_t v) { return this->internal_2_external[v]; },
        max_iterations);
    auto external_ids = translate<uint64_t>(clusters);
