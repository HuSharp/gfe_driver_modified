...
#pragma omp parallel for schedule(dynamic, 64)
    for (uint64_t v = 0; v < nv; v++)
    {
        ...
        if (stinger_vtype_get(STINGER, v) == 1)
            continue; // node marked for deletion
...
uint64_t Stinger::lcc_count_intersections(
    int64_t vertex1,
    int64_t vertex2,
    ...
    int64_t vertex1_neighbours_sz)
{
    ...
    auto binsearch = [&, vertex1_neighbours, vertex1_neighbours_sz](int64_t vertex3) {
        ...
        while (first <= last)
        {
            ...
            {
                COUT_DEBUG(
                    "Triangle " << get_external_id(vertex1) << " - " << get_external_id(vertex2) << " - "
                                << get_external_id(vertex3));
                return true;
            }
            else
            {
