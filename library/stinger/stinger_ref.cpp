...
// Stinger stores the weights as int64_t, these macros handle the conversion to
// double
...
StingerRef::StingerRef(bool directed)
    : Stinger(directed)
{}
...
#pragma omp parallel for reduction(+ : awake_count) schedule(dynamic, 1024)
    for (int64_t u = 0; u < NV; u++)
    {
        ...
        { // the node has not been visited yet
            bool skip = false;
            STINGER_FORALL_IN_EDGES_OF_VTX_BEGIN(g, u)
            { // check whether any of its incoming neighbours has already
                // been visited
                if (!skip && front.get_bit(STINGER_EDGE_DEST))
                {
                    //                    parent[u] = STINGER_EDGE_DEST; // if we were
...
static int64_t TDStep(stinger_t * g, pvector<int64_t> & distances, int64_t distance, SlidingQueue<int64_t> & queue)
{
...
static void BitmapToQueue(stinger_t * g, int64_t NV, const Bitmap & bm, SlidingQueue<int64_t> & queue)
{
#pragma omp parallel
    {
        QueueBuffer<int64_t> lqueue(queue);
...
static pvector<int64_t> InitDistances(stinger_t * g, int64_t NV)
{
...
void StingerRef::bfs(uint64_t source_external_id, const char * dump2file)
{
    ...
    common::Timer timer;
    timer.start();
    //    uint64_t nv = stinger_max_active_vertex(STINGER); // it doesn't register
    //    the coefficient if the last vertices are isolated
    ...
    // translate the vertex ids and set the distance of the non reached vertices
    // to max()
...
static pvector<double> do_pagerank(
    stinger_t * G,
    uint64_t num_active_vertices,
    uint64_t num_total_vertices,
...
#pragma omp parallel for reduction(+ : dangling_sum)
        ...
        //        COUT_DEBUG("[" << iteration << "] base score: " << base_score <<
...
void StingerRef::pagerank(uint64_t num_iterations, double damping_factor, const char * dump2file)
{
    ...
    // execute the algorithm
    auto result
        = do_pagerank(STINGER, num_active_vertices, num_registered_vertices, num_iterations, damping_factor, tcheck);
    if (tcheck.is_timeout())
    {
        RAISE_EXCEPTION(TimeoutError, "Timeout occurred after " << timer);
    }
...
static // do_wcc
    pvector<uint64_t>
    do_wcc(stinger_t * G, uint64_t num_total_vertices, utility::TimeoutService & timer)
{
    // init
