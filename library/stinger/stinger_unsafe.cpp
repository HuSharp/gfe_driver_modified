...
#define COUT_DEBUG_FORCE(msg)                                                  \
    {                                                                          \
        ...
        std::cout << "[Stinger::" << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
...
#endif
    config.no_map_none_etype = config.no_map_none_vtype
        = 1; // I think this is whether we want to attach names (as strings) to the
    // vertex/edge types, 1 = we don't use this feature
    config.no_resize = 0; // when building stinger for the first time, allow to resize the
        // internal structs if they do not fit into memory, 0 = allow resize

    m_stinger_graph = stinger_new_full(&config);
    assert(m_stinger_graph != nullptr && "Stinger allocation");
    if (m_stinger_graph == nullptr)
        ERROR("Cannot allocate stinger graph");
}

Stinger::~Stinger()
{
    stinger_free_all(STINGER);
    m_stinger_graph = nullptr;
}
...
// @return a value >= 0 than if it's the index in the stinger adjacency list, or
// -1 in case of error
...
template <typename T>
vector<pair<uint64_t, T>> Stinger::to_external_ids(const T * __restrict internal_ids, size_t internal_ids_sz)
{
    vector<pair<uint64_t, T>> external_ids(internal_ids_sz);

#pragma omp parallel for
    for (uint64_t internal_id = 0; internal_id < internal_ids_sz; internal_id++)
    {
        if (stinger_vtype_get(STINGER, internal_id) == 0)
        { // if = 1, the node is marked for deletion
            char * vertex_id_name = nullptr;
            uint64_t vertex_id_name_length = 0;
...
int64_t Stinger::get_or_create_vertex_id(uint64_t external_vertex_id)
{
    ...
    // @return 0 on existing mapping found. 1 on new mapping created. -1 on
    // failure (STINGER is full).
...
#pragma omp parallel for
    for (uint64_t internal_id = 0; internal_id < internal_ids_sz; internal_id++)
    {
        ...
        //        COUT_DEBUG("internal_id: " << internal_id << " -> " <<
        //        external_id);
...
template <typename T>
vector<pair<uint64_t, T>> Stinger::to_external_ids(const vector<T> & internal_ids)
{
    return to_external_ids(internal_ids.data(), internal_ids.size());
}

template vector<pair<uint64_t, double>> Stinger::to_external_ids<double>(const vector<double> & internal_ids);
template vector<pair<uint64_t, double>> Stinger::to_external_ids<double>(
    const double * __restrict internal_ids,
    size_t internal_ids_sz);
template vector<pair<uint64_t, int64_t>> Stinger::to_external_ids<int64_t>(const vector<int64_t> & internal_ids);
template vector<pair<uint64_t, int64_t>> Stinger::to_external_ids<int64_t>(
    const int64_t * __restrict internal_ids,
    size_t internal_ids_sz);
template vector<pair<uint64_t, uint64_t>> Stinger::to_external_ids<uint64_t>(const vector<uint64_t> & internal_ids);
template vector<pair<uint64_t, uint64_t>> Stinger::to_external_ids<uint64_t>(
    const uint64_t * __restrict internal_ids,
    size_t internal_ids_sz);
...
uint64_t Stinger::num_edges() const
{
    ...
    if (!m_directed)
        num_directed_edges /= 2; /* divided by two, because we always store the edge
...
bool Stinger::add_vertex(uint64_t vertex_id)
{
    ...
    // Stinger has an internal dictionary where the vertex_id from the external
    // are mapped into internal indices for the internal adjacency list. Here, we
    // simply register the mapping to the dictionary.
    ...
    case 0: // the mapping already exists
        return false;
        //    case 0: {
        //        // because we cannot delete an existing mapping, here we use the
        //        convention of setting a vertex type to 1
        //        // if the vertex is supposed to be deleted
        //        scoped_lock<SpinLock> lock(m_spin_lock);
        //
        //        vtype_t type = stinger_vtype_get(STINGER, internal_vertex_id);
        //        switch(type){
        //        case 0:// the mapping already existed and the vertex is active
        //        (type = 0)
        //            return false;
        //        case 1: // the mapping already existed but the vertex was deleted
        //        (type = 1)
        //            stinger_vtype_set(STINGER, internal_vertex_id, 0); // reset to
        //            active m_num_vertices ++; return true;
        //        default:
...
bool Stinger::remove_vertex(uint64_t external_vertex_id)
{
    ...
    // It should also remove the mapping, but the current Stinger implementation
    // does not support this, returning -1.
    ...
    //    int64_t internal_vertex_id = get_internal_id(external_vertex_id);
    //    if(internal_vertex_id < 0) return false; // the vertex does not even
    //    have a mapping
    //
    //    // remove all edges (directed and undirected) for the
    //    internal_vertex_id.
    //    // It should also remove the mapping, but the current Stinger
    //    implementation does not support this, returning -1.
    //    // As workaround, we set the vertex type to 1, to signal the vertex as
    //    inactive auto rc = stinger_remove_vertex(STINGER, internal_vertex_id);
    //
    //    scoped_lock<SpinLock> lock(m_spin_lock);
    //    if(rc == -1){
    //        if(stinger_vtype_get(STINGER, internal_vertex_id) == 1){ // already
    //        previously marked as inactive
    //            return false;
    //        } else {
...
bool Stinger::add_vertex(uint64_t vertex_id)
{
    ...
    else
    {
        ...
        {
            mapping = m_reuse_vertices.back();
            m_reuse_vertices.pop_back();
        }
        else
        {
...
bool Stinger::remove_vertex(uint64_t external_vertex_id)
{
    int64_t internal_vertex_id = get_internal_id(external_vertex_id);
    if (internal_vertex_id < 0)
        return false; // the vertex does not even have a mapping
...
void Stinger::dump_ostream(ostream & out) const
{
    ...
    out << "[STINGER] Vertices: " << num_vertices() << ", edges: " << num_edges() << ", directed: " << std::boolalpha
        << is_directed() << ", size: " << stinger_graph_size(graph) << " bytes"
        << "\n";
    for (int64_t internal_vertex_id = 0, vertex_max = stinger_max_active_vertex(graph);
         internal_vertex_id <= vertex_max;
         internal_vertex_id++)
    {
        if (stinger_vtype_get(graph, internal_vertex_id) == 1)
            continue; // vertex flagged as inactive
        ...
        out << "type: " << dump_vertex_type(graph, internal_vertex_id)
            << ", weight: " << stinger_vweight_get(graph, internal_vertex_id) << ", "
            << "degree in/out/total: " << stinger_indegree_get(graph, internal_vertex_id) << "/"
            << stinger_outdegree_get(graph, internal_vertex_id) << "/" << stinger_degree_get(graph, internal_vertex_id);
