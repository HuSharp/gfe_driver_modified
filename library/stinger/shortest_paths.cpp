...
#define CHECK_TIMEOUT                                                                                       \
    if (timeout.count() > 0 && (chrono::steady_clock::now() - time_start) > timeout)                        \
    {                                                                                                       \
        RAISE_EXCEPTION(                                                                                    \
            TimeoutError,                                                                                   \
...
bool static comp(weighted_vertex_t a, weighted_vertex_t b)
{
...
void Stinger::compute_shortest_paths(uint64_t source_external_id, bool weighted, const char * dump2file)
{
    ...
    if (source_internal_id < 0)
        ERROR("The vertex `" << source_external_id << "' does not exist");
    auto result = a_star(
        STINGER,
        stinger_max_active_vertex(STINGER) + 1,
        source_internal_id,
        /* all vertices */ -1,
        /* ignore weights */ !weighted,
