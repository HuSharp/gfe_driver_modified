...
#define COUT_DEBUG_FORCE(msg)                                                                                      \
    {                                                                                                              \
        scoped_lock<mutex> lock(::gfe::_log_mutex);                                                                \
        std::cout << "[LLAMARef::" << __FUNCTION__ << "] [" << common::concurrency::get_thread_id() << "] " << msg \
                  << std::endl;                                                                                    \
    }
#if defined(DEBUG)
...
LLAMARef::LLAMARef(bool directed)
    : LLAMAClass(directed)
{}
 ...
 *                                                                            *
 *****************************************************************************/
static uint64_t _llama_outdegree(ll_mlcsr_ro_graph & snapshot, bool is_directed, int64_t llama_vertex_id)
{
    if (is_directed)
    {
...
static int64_t BUStep(
    ll_mlcsr_ro_graph & graph,
    bool is_directed,
    pvector<int64_t> & distances,
    int64_t distance,
    Bitmap & front,
    Bitmap & next)
{
...
#pragma omp parallel for reduction(+ : awake_count) schedule(dynamic, 1024)
    for (int64_t u = 0; u < graph.max_nodes(); u++)
    {
        ...
        { // the node has not been visited yet
            ...
            bool done = false;
            for (edge_t e = graph.in_iter_next_fast(iterator); e != LL_NIL_EDGE && !done;
            ...
            if (!is_directed && !done)
            { // the actual set of edges in undirected graphs is composed
                // by both LLAMA's incoming and outgoing edges
                graph.out_iter_begin(iterator, u);
                for (edge_t e = graph.out_iter_next(iterator); e != LL_NIL_EDGE && !done;
                     e = graph.out_iter_next(iterator))
                {
                    node_t n = LL_ITER_OUT_NEXT_NODE(*G, iterator, e);
                    if (front.get_bit(n))
                    {
...
static void BitmapToQueue(ll_mlcsr_ro_graph & graph, const Bitmap & bm, SlidingQueue<int64_t> & queue)
{
#pragma omp parallel
    {
        QueueBuffer<int64_t> lqueue(queue);
...
static pvector<int64_t> InitDistances(ll_mlcsr_ro_graph & graph, bool is_directed)
{
...
void LLAMARef::bfs(uint64_t external_source_vertex_id, const char * dump2file)
{
    ...
    for (uint64_t i = 0; i < graph.num_levels(); i++)
    {
        num_edges += graph.max_edges(i);
    }

    slock.unlock(); // here we lose the ability to refer to m_vmap_read_only from
        // now on
...
static pvector<double> do_pagerank(
    ll_mlcsr_ro_graph & graph,
    uint64_t current_num_vertices,
    bool is_directed,
...
#pragma omp parallel for reduction(+ : dangling_sum)
        for (uint64_t v = 0; v < graph.max_nodes(); v++)
        {
            ...
            if (graph.out_degree(v) + graph.in_degree(v) == 0)
                continue;
...
static // do_wcc
    pvector<uint64_t>
    do_wcc(ll_mlcsr_ro_graph & graph, bool is_directed, utility::TimeoutService & timer)
{
    // init
    ...
    utility::TimeoutService & timer)
{
    ...
    for (int i = 0; i < graph.num_levels(); i++)
    {
        num_total_edges += graph.max_edges(i);
    }
    ll_mlcsr_edge_property<double> & weights
        = *reinterpret_cast<ll_mlcsr_edge_property<double> *>(graph.get_edge_property_64(g_llama_property_weights));
            ...
            for (size_t i = 0; i < curr_frontier_tail; i++)
            {
                ...
                if (dist[u] >= delta * static_cast<double>(curr_bin_index))
                {
                    ...
                    { // the actual set of edges in undirected graphs is
                        ...
                        // edges
                        graph.in_iter_begin_fast(iterator, u);
                        for (edge_t s_idx = graph.in_iter_next_fast(iterator); s_idx != LL_NIL_EDGE;
                             s_idx = graph.in_iter_next_fast(iterator))
                        {
                            node_t v = LL_ITER_OUT_NEXT_NODE(*G, iterator, e);
