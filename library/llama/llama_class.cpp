...
void llama_profile_print_stats()
{
    ...
    COUT_DEBUG_FORCE(
        "Cumulative time spent in #build: " << common::time::to_string(chrono::nanoseconds(g_llama_build_nanosecs))
                                            << ", percentage: " << oh_build << "%");

    double oh_add_vertex = g_llama_add_vertex_nanosecs * 100.0 / total_time_nanosecs;
    COUT_DEBUG_FORCE(
        "Cumulative time spent in #add_vertex: "
        << common::time::to_string(chrono::nanoseconds(g_llama_add_vertex_nanosecs))
        << ", percentage: " << oh_add_vertex << "%")

    double oh_add_node = g_llama_add_node_nanosecs * 100.0 / g_llama_add_vertex_nanosecs;
    COUT_DEBUG_FORCE(
        "Cumulative time spent in ll_writable_graph::add_node: "
        << common::time::to_string(chrono::nanoseconds(g_llama_add_node_nanosecs))
        << ", overhead w.r.t. #add_vertex: " << oh_add_node << "%");

    double oh_add_edge = g_llama_add_edge_total_nanosecs * 100.0 / total_time_nanosecs;
    COUT_DEBUG_FORCE(
        "Cumulative time spent in ll_writable_graph::add_edge_if_not_exists: "
        << common::time::to_string(chrono::nanoseconds(g_llama_add_edge_total_nanosecs))
        << ", percentage: " << oh_add_edge << "%");

    double oh_add_edge_check = g_llama_add_edge_check_nanosecs * 100.0 / g_llama_add_edge_total_nanosecs;
    COUT_DEBUG_FORCE(
        "Cumulative time spent checking edge existence in "
        "ll_writable_graph::add_edge_if_not_exists: "
        << common::time::to_string(chrono::nanoseconds(g_llama_add_edge_check_nanosecs))
        << ", overhead: " << oh_add_edge_check << "%");
}
#endif
...
LLAMAClass::LLAMAClass(bool is_directed)
    : m_is_directed(is_directed)
{
    m_db = new ll_database();

    auto & csr = m_db->graph()->ro_graph();
    csr.create_uninitialized_node_property_64(
        g_llama_property_names,
        LL_T_INT64); // translate a logical id (e.g. node_id = 4) into an external
    // node id (e.g. user_id = 21084718954)
...
double LLAMAClass::get_weight(uint64_t ext_source_id, uint64_t ext_destination_id) const
{
...
#if defined(LLAMA_HASHMAP_WITH_TBB)
    decltype(m_vmap)::const_accessor accessor1,
        accessor2; // shared lock on the dictionary
    if (!m_vmap.find(accessor1, ext_source_id))
    {
        return nan;
    }
    if (!m_vmap.find(accessor2, ext_destination_id))
    {
        return nan;
    }
    ...
    { // for undirected graphs, ensure src_id < dst_id
        std::swap(source, destination);
    }

    edge_t edge_id = m_db->graph()->find(source, destination);
    if (edge_id == LL_NIL_EDGE)
        return nan; // the edge does not exist
    ...
    try
    {
        ...
        int_source_id = vmap_write_store_find(ext_source_id); // throws std::out_of_range if
            // ext_source_id is not present
...
int64_t LLAMAClass::vmap_write_store_find(uint64_t external_vertex_id) const
{
    ...
    if (m_vmap_updated.find(external_vertex_id, logical_id))
    { // found
        return logical_id;
    }

    if (m_vmap_removed.contains(external_vertex_id))
    { // vertex explicitly removed
        throw std::out_of_range("vertex deleted");
    }

    if (m_vmap_read_only.find(external_vertex_id, logical_id))
    { // found
        return logical_id;
    }
...
int64_t LLAMAClass::get_internal_vertex_id(uint64_t external_vertex_id) const
{
    ...
    if (!m_vmap_read_only.find(external_vertex_id, llama_source_vertex_id))
    { // side effect, it assigns
        // llama_source_vertex_id
        ERROR("The given vertex does not exist: " << external_vertex_id);
    }
#endif
}
...
bool LLAMAClass::add_vertex(uint64_t vertex_id_ext)
{
    ...
    if (inserted)
    {
        ...
        m_db->graph()
            ->get_node_property_64(g_llama_property_names)
            ->set(
                node_id,
                vertex_id_ext); // register the mapping internal -> external
    ...
    if (!vmap_write_store_contains(vertex_id_ext))
    {
        ...
        /**
     * Here there may be a race condition, where both T1 and T2 invoke
     * m_db->graph()->add_node(); However we roll back the effects of the second
     * thread in #m_vmap_updated.upsert. The lambda is invoked if a key is
     * already present in the hash table (the node_id set by T1).
     */
        bool inserted = true;
        m_vmap_updated.upsert(
            vertex_id_ext,
            [this, node_id, &inserted](int64_t & previous_node_id /* ignore */) {
                // roll back
...
int64_t LLAMAClass::get_or_create_vertex_id(uint64_t vertex_id_ext)
{
     ...
     */
        m_vmap_updated.upsert(
            vertex_id_ext,
            [this, &node_id](int64_t previous_node_id) {
...
bool LLAMAClass::add_edge(graph::WeightedEdge e)
{
    ...
    decltype(m_vmap)::const_accessor accessor1,
        accessor2; // shared lock on the dictionary
    if (!m_vmap.find(accessor1, e.source()))
    {
        return false;
    }
    if (!m_vmap.find(accessor2, e.destination()))
    {
        return false;
    }
    ...
    try
    {
        ...
        llama_destination_id = vmap_write_store_find(e.m_destination); // throws std::out_of_range if
            // ext_source_id is not present
    }
    catch (std::out_of_range & e)
    {
...
{
    if (!m_is_directed)
    { // for undirected graphs, ensure llama_source_id <
        // llama_destination_id
...
bool LLAMAClass::remove_edge(graph::Edge e)
{
    ...
    // Again, this cannot be strictly correct, a race condition can still occur
    // where a vertex is removed in the meanwhile we are changing an edge
    if (!m_is_directed)
    { // for undirected graphs, ensure llama_source_id <
        // llama_destination_id
        if (llama_source_id > llama_destination_id)
...
#pragma omp parallel for
    for (node_t llama_node_id = 0; llama_node_id < N; llama_node_id++)
    {
        ...
        // this is a bit of a stretch: the impl~ from llama assumes that a node does
        // not exist only if it does not have any incoming or outgoing edges.
        if (!graph.node_exists(llama_node_id))
            continue;
...
#define INSTANTIATE_TRANSLATE(TYPE)                                    \
    template vector<pair<uint64_t, TYPE>> LLAMAClass::translate<TYPE>( \
        ll_mlcsr_ro_graph & graph,                                     \
        const TYPE * __restrict data);                                 \
    template void LLAMAClass::save_results<TYPE, true>(                \
        const vector<pair<uint64_t, TYPE>> & result,                   \
        const char * dump2file);                                       \
    template void LLAMAClass::save_results<TYPE, false>(               \
        const vector<pair<uint64_t, TYPE>> & result,                   \
        const char * dump2file);
...
{
    auto names
        = reinterpret_cast<ll_mlcsr_node_property<uint64_t> *>(graph.get_node_property_64(g_llama_property_names));

    out << "[LLAMA] num vertices (global): " << num_vertices() << ", num edges (global): " << num_edges()
        << ", max nodes: " << graph.max_nodes() << ", num levels (#snapshots): " << graph.num_levels() << endl;
    ...
    for (node_t node_id = 0; node_id < graph.max_nodes(); node_id++)
    {
        ...
        out << "[" << node_id << " -> " << names->get(node_id) << "] " << get_write_store_outdegree(node_id)
            << " outgoing edges: ";
...
// ll_writable_graph
template void LLAMAClass::dump_impl<ll_mlcsr_ro_graph>(std::ostream & out, ll_mlcsr_ro_graph & graph) const;
template void LLAMAClass::dump_impl<ll_writable_graph>(std::ostream & out, ll_writable_graph & graph) const;
