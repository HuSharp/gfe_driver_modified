...
#define COUT_DEBUG_FORCE(msg)                                                    \
    {                                                                            \
        ...
        std::cout << "[Interface::" << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
...
ImplementationManifest::ImplementationManifest(
    ...
    unique_ptr<Interface> (*factory)(bool))
    : m_name(name)
    , m_description(description)
    , m_factory(factory)
{}

std::unique_ptr<Interface> generate_baseline_adjlist(bool directed_graph)
...
#if defined(HAVE_GRAPHONE)
// Heuristics to set the max number of vertices that the implementation can
// sustain. We wish to create up to 4G vertices, but unfortunately this limit is
// too high for usage in a workstation machines, so we decrease the value
...
static uint64_t graphone_max_num_vertices()
{
    ...
    { // critical section, for the output
        ...
        cout << "GraphOne: capacity of the vertex array implicitly set to: " << common::ComputerQuantity(num_vertices)
             << " vertices " << endl;
    }
...
std::unique_ptr<Interface> generate_graphone_cons_sp(bool directed_graph)
{
    ...
    return unique_ptr<Interface>{new GraphOne(
        directed_graph,
        /* vtx dict ? */ true,
        /* blind writes ? */ false,
        /* ignore build ? */ false,
...
#if defined(HAVE_LIVEGRAPH)
std::unique_ptr<Interface> generate_livegraph_ro(bool directed_graph)
{ // read only transactions for Graphalytics
    return unique_ptr<Interface>(new LiveGraphDriver(directed_graph, /*read_only ? */ true));
}
std::unique_ptr<Interface> generate_livegraph_rw(bool directed_graph)
{ // read-write transactions for Graphalytics
...
std::unique_ptr<Interface> generate_teseo_lcc(bool directed_graph)
{ // LCC custom algorithm
...
vector<ImplementationManifest> implementations()
{
    ...
    // v2 25/06/2020: Updates, implicitly create a vertex referred in a new edge
    // upon first reference with the method add_edge_v2 v3 14/04/2021: Fix the
    // predicate in the TimeoutService
    ...
    result.emplace_back(
        "csr3-numa",
        "CSR baseline, allocate the internal arrays using all NUMA nodes",
        &generate_csr_numa);
    result.emplace_back(
        "csr3-lcc-numa",
        "CSR baseline, allocate the internal arrays using all "
        "NUMA nodes, sort-merge impl for the LCC kernel",
        &generate_csr_lcc_numa);

    // Temporary, we run csr3-lcc on a single NUMA node to pin down if NUMA
    // effects are resposible for SortedVectorAL being faster some times
    result.emplace_back(
        "single-numa-node-csr3-lcc",
        "CSR baseline, sort-merge impl for the LCC kernel",
        &generate_csr_lcc);
    ...
    // v3 23/01/2020: switch to Intel TBB for the vertex dictionary. All
    // experiments should be repeated. v4 13/05/2020: fair mutex for compactation,
    // it's a major bug fix as new delta levels were not issued every 10s due to
    // starvation. All experiments should be repeated v5 12/06/2020: OMP dynamic
    // scheduling in the Graphalytics kernels v6 25/06/2020: Updates, implicitly
    // create a vertex referred in a new edge upon first reference with the method
    // add_edge_v2 v7 14/04/2021: Fix the predicate in the TimeoutService v8
    // 23/04/2021: Materialization step with a vector
...
#if defined(HAVE_GRAPHONE)
    // v2 11/06/2020: Bug fix for the properties on the static views + OMP dynamic
    // scheduling. Repeat all experiments for Graphalytics. v3 25/06/2020:
    // Updates, implicitly create a vertex referred in a new edge upon first
    // reference with the method add_edge_v2 v4 27/06/2020: Fix the number of
    // vertices. It only affects the variations with dense vertices. v5
    // 14/04/2021: Fix the predicate in the TimeoutService v6 23/04/2021:
    // Materialization step with a vector
    result.emplace_back(
        "g1_v6-cons-sp",
        "GraphOne, consistency for updates, sparse vertices (vertex dictionary)",
        &generate_graphone_cons_sp);
    result.emplace_back(
        "g1_v6-cons-dv",
        "GraphOne, consistency for updates, dense vertices",
        &generate_graphone_cons_dv);
    result.emplace_back(
        "g1_v6-bw-sp",
        "GraphOne, blind writes, sparse vertices (vertex dictionary)",
        &generate_graphone_bw_sp);
    result.emplace_back("g1_v6-bw-dv", "GraphOne, blind writes, dense vertices", &generate_graphone_bw_dv);
    result.emplace_back(
        "g1_v6-bw-sp-ignore-build",
        "GraphOne, blind writes, sparse vertices (vertex dictionary), new "
        "deltas/levels cannot be explicitly created",
        &generate_graphone_bw_sp_ignore_build);
    result.emplace_back(
        "g1_v6-bw-dv-ignore-build",
        "GraphOne, blind writes, dense vertices, new "
        "deltas/levels cannot be explicitly created",
        &generate_graphone_bw_dv_ignore_build);
    result.emplace_back(
        "g1_v6-ref",
        "GraphOne, reference GAP BS for the Graphalytics algorithms",
        &generate_graphone_ref);
    result.emplace_back(
        "g1_v6-ref-ignore-build",
        "GraphOne, reference GAP BS for the Graphalytics algorithms",
        &generate_graphone_ref_ignore_build);
#endif
    ...
    result.emplace_back(
        "livegraph3_ro",
        "LiveGraph, use read-only transactions for the Graphalytics kernels",
        &generate_livegraph_ro);
    result.emplace_back(
        "livegraph3_rw",
        "LiveGraph, use read-write transactions for the Graphalytics kernels",
        &generate_livegraph_rw);
#endif

#if defined(HAVE_TESEO)
    // v1 05/04/2020: initial version for evaluation
    // v2 28/04/2020: big rewrite: dense file, delayed rebalances, new leaf
    // layout, new rebalancer logic. All experiments should be repeated, that is,
    // ignore v1. v3 27/05/2020: scan enhancements: AUX view, prefetching, NUMA,
    // segment's pivot, direct pointers in the AUX view. All graphalytics & bm
    // experiments should be repeated. v4 15/06/2020: cursor state + support for
    // R/W iterators. It only affects scans (Graphalytics & bm) v5 26/06/2020:
    // updates, implicitly create a vertex referred in a new edge upon first
    // reference with the method add_edge_v2 v6 18/07/2020: vertex table v7
    // 08/11/2020: vertical partitioning v8 23/11/2020: variable length leaves v9
    // 07/01/2021: bug fixes v10 08/01/2021: set the thread affinity by default
    // v11 14/04/2021: Fix the predicate in the TimeoutService
    // v12 19/04/2021: Materialization with a vector (12b, bogus translation IDs
    // in TeseoRealVertices) v13 08/12/2021: Vertex table: uses pseudorandom hash
    // function to avoid clustering on en-wiki dataset
    result.emplace_back("teseo.13", "Teseo", &generate_teseo);
    result.emplace_back(
        "teseo-rw.13",
        "Teseo. Use read-write transactions for graphalytics, to "
    ...
    result.emplace_back(
        "teseo-lcc-dv.13b",
        "Teseo, dense vertices and sort-merge implementation of the LCC kernel",
        &generate_teseo_real_vtx_lcc);
#endif
   ...
   *   - BFS
   *     - does not use edge_count anymore but sums up while populating the
   *distance array
   *     - removes unecessary double translation
   *   - LCC
   *     - uses now shared locks
   *     - uses the GFE driver implementation which is based on pthreads and not
   *OpenMP like mine
   *   - SSSP
   ...
   *   - Skiplists
   *     - uses the correct probability function for deciding the height of new
   *nodes
   **/
   ...
   * - BFS is not run as first analytics algorithms. Therefore, GC is not
   * running as the first run.
   * - PageRank is run first.
   */
    //    result.emplace_back("sortledton.lock-free-analytics", "Sortledton",
    //    &generate_sortledton);

    /**
   * Temporary version.
   *  - uses raw memory access for the BFS bottom up step and small sets. No
   * locks acquired.
   *  - do not use ANY of these results, I ran many experiments with this
   * version.
   */
    //    result.emplace_back("sortledton.raw-access-bfs", "Sortledton",
    //    &generate_sortledton);
   ...
   */
    //    result.emplace_back("sortledton-no-size-versioning", "Sortledton",
    //    &generate_sortledton);
    /**
   * Sortledton with steam gc implementation for adjacency set sizes.
   */
    //    result.emplace_back("sortledton-steam-gc", "Sortledton",
    //    &generate_sortledton);
    /**
   * Sorltedton with steam gc implementation for adjacency set sizes and edge
   * version chains.
   *
   * Measurements contained some debug code for the isolated workloads. We
   * measure these again.
   */
    //    result.emplace_back("sortledton-steam-gc-1", "Sortledton",
    //    &generate_sortledton); result.emplace_back("sortledton-steam-gc-2",
    //    "Sortledton", &generate_sortledton);
    /**
...
//    &generate_sortledton);
...
#if defined(HAVE_MICROBENCHMARKS)
    //    result.emplace_back("vector_al", "Vector adjacency lists",
    //    &generate_microbenchmarks); result.emplace_back("sorted_vector_al",
    //    "Sorted Vector adjacency lists", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_sorted_vector_al", "Sorted Vector
    //    adjacency lists with robin hood hash set index",
    //    &generate_microbenchmarks); result.emplace_back("tree_sorted_vector_al",
    //    "Sorted Vector adjacency lists with std::ordered_map index",
    //    &generate_microbenchmarks); result.emplace_back("robin_hood_al",
    //    "Adjacency set based on a flat robin hood hash set.",
    //    &generate_microbenchmarks);
    //    result.emplace_back("edgeiter_sorted_vector_al", "Sorted Vector
    //    adjacency lists with EdgeIterator instead of BlockedEdgeIterator",
    //    &generate_microbenchmarks);

    // Bugfix 14.06.2021 library is now linked against OpenMP and runs algorithms
    // in paralell, also adds a robin hood based adjacency list
    //    result.emplace_back("vector_al.1", "Vector adjacency lists",
    //    &generate_microbenchmarks); result.emplace_back("sorted_vector_al.1",
    //    "Sorted Vector adjacency lists", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_sorted_vector_al.1", "Sorted Vector
    //    adjacency lists with robin hood hash set index",
    //    &generate_microbenchmarks);
    //    result.emplace_back("tree_sorted_vector_al.1", "Sorted Vector adjacency
    //    lists with std::ordered_map index", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_al.1", "Adjacency set based on a flat
    //    robin hood hash set.", &generate_microbenchmarks);
    //    result.emplace_back("edgeiter_sorted_vector_al.1", "Sorted Vector
    //    adjacency lists with EdgeIterator instead of BlockedEdgeIterator",
    //    &generate_microbenchmarks);

    // Bugfix 16.06.2021 vertices addtions are now serialized due to issues with
    // TBB concurent_hash_set semantics. Changes to shared locks to speed up LCC.
    // Also, adds a special LCC method for hash sets.
    //    result.emplace_back("vector_al.2", "Vector adjacency lists",
    //    &generate_microbenchmarks); result.emplace_back("sorted_vector_al.2",
    //    "Sorted Vector adjacency lists", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_sorted_vector_al.2", "Sorted Vector
    //    adjacency lists with robin hood hash set index",
    //    &generate_microbenchmarks);
    //    result.emplace_back("tree_sorted_vector_al.2", "Sorted Vector adjacency
    //    lists with std::ordered_map index", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_al.2", "Adjacency set based on a flat
    //    robin hood hash set.", &generate_microbenchmarks);
    //    result.emplace_back("edgeiter_sorted_vector_al.2", "Sorted Vector
    //    adjacency lists with EdgeIterator instead of BlockedEdgeIterator",
    //    &generate_microbenchmarks);
    //
    //    result.emplace_back("single-numa-node-sorted-vector_al.2", "Sorted
    //    Vector adjacency lists with EdgeIterator instead of
    //    BlockedEdgeIterator", &generate_microbenchmarks);
    ...
    //   * Removes locks from sequential tree and hash set index when running
    //   analytics
    //    result.emplace_back("vector_al.3", "Vector adjacency lists",
    //    &generate_microbenchmarks); result.emplace_back("sorted_vector_al.3",
    //    "Sorted Vector adjacency lists", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_sorted_vector_al.3", "Sorted Vector
    //    adjacency lists with robin hood hash set index",
    //    &generate_microbenchmarks);
    //    result.emplace_back("tree_sorted_vector_al.3", "Sorted Vector adjacency
    //    lists with std::ordered_map index", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_al.3", "Adjacency set based on a flat
    //    robin hood hash set.", &generate_microbenchmarks);
    //    result.emplace_back("edgeiter_sorted_vector_al.3", "Sorted Vector
    //    adjacency lists with EdgeIterator instead of BlockedEdgeIterator",
    //    &generate_microbenchmarks);
    //   result.emplace_back("mb-csr.3", "CSR data structure of micro benchmarks",
    //   &generate_microbenchmarks);

    //      result.emplace_back("single-numa-node-sorted-vector_al.4", "Sorted
    //      Vector adjacency lists run on a single NUMA node",
    //      &generate_microbenchmarks);
    ...
    //    result.emplace_back("mb-csr.6", "CSR data structure of micro
    //    benchmarks", &generate_microbenchmarks);
    //    result.emplace_back("vector_al.4", "Vector adjacency lists",
    //    &generate_microbenchmarks);
    result.emplace_back("sorted_vector_al.6", "Sorted Vector adjacency lists", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_sorted_vector_al.4", "Sorted Vector
    //    adjacency lists with robin hood hash set index",
    //    &generate_microbenchmarks);
    //    result.emplace_back("tree_sorted_vector_al.4", "Sorted Vector adjacency
    //    lists with std::ordered_map index", &generate_microbenchmarks);
    //    result.emplace_back("robin_hood_al.4", "Adjacency set based on a flat
    //    robin hood hash set.", &generate_microbenchmarks);
    //    result.emplace_back("edgeiter_sorted_vector_al.4", "Sorted Vector
    //    adjacency lists with EdgeIterator instead of BlockedEdgeIterator",
    //    &generate_microbenchmarks);

    result.emplace_back(
        "single-numa-node-sorted-vector_al.4",
        "Sorted Vector adjacency lists run on a single NUMA node",
        &generate_microbenchmarks);

    // Test this is the same as mb-csr.6 but with WCC implemented and compiled as
    // part of the GFE driver.
    //      result.emplace_back("mb-csr.7", "CSR data structure of micro
    //      benchmarks", &generate_microbenchmarks);
...
void Interface::dump(const std::string & path) const
{
    ...
    if (!handle.good())
        ERROR("[dump] Cannot open the file: `" << path << "'");
    dump_ostream(handle);
    handle.close();
}
...
bool UpdateInterface::batch(const SingleUpdate * array, size_t array_sz, bool force)
{
    ...
    if (force)
    {
        // now a bit of a hack, we want all updates to succeed. An update may fail
        // if a vertex is still being added by another thread in the meanwhile
        for (uint64_t i = 0; i < array_sz; i++)
        {
            ...
            { // insert
                ...
                if (!add_edge(edge))
                { // avoid infinite loops/waits
                    auto op = [this](graph::WeightedEdge edge) {
