...
#define COUT_DEBUG_FORCE(msg)                                                                         \
    {                                                                                                 \
        ...
        std::cout << "[" << COUT_DEBUG_CLASSNAME << "::" << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
...
// 16 and 129 below are arbitrary values, enough to represent the current
// processor sizes
using processors_t = int[16][129]; // up to 16 sockets, the first value is the
    // number of processors in socket
...
static void parse_cpuinfo(int & num_sockets, processors_t & processors)
{
    ...
    if (file == nullptr)
    {
        fprintf(
            stderr,
            "[parse_cpuinfo] ERROR: cannot open /proc/cpuinfo, errno: %d, "
            "error: %s\n",
            errno,
            strerror(errno));
        ::exit(EXIT_FAILURE);
    ...
    while (fgets(buffer, buffer_sz, file) != nullptr)
    {
        ...
        uint64_t key_len = 0;
        while (key_len < buffer_len && buffer[key_len] != ':')
        {
            key_len++;
        }
        if (key_len == buffer_len)
            continue; // no key read

        if (read_processor_id)
        { // read the processor
            if (key_len < strlen("processor"))
                continue; // next line
        ...
        { // read the physical id
            ...
            if (strncmp("physical id", buffer, strlen("physical id")) == 0)
            {
                ...
                if (socket_id >= max_num_processors)
                {
                    fprintf(
                        stderr,
                        "[parse_cpuinfo] ERROR: invalid socket ID: %d, we can "
                ...
                { // assume the sockets are numbered 0, 1, 2, ...
                ...
                if (processors[socket_id][0] >= max_num_processors)
                {
                    fprintf(
                        stderr,
                        "[parse_cpuinfo] ERROR: already reached the max number of "
                        "processors (%d) for the socket %d\n",
                        max_num_processors,
                        socket_id);
                    exit(EXIT_FAILURE);
                }
    ...
    if (!feof(file))
    { // we expect to have read the whole file
        fprintf(
            stderr,
            "[parse_cpuinfo] ERROR: cannot parse /proc/cpuinfo, errno: %d, "
            "error: %s\n",
            errno,
            strerror(errno));
        exit(EXIT_FAILURE);
...
ThreadAffinity::ThreadAffinity(bool enabled)
    ...
    if (m_is_enabled)
    {
        ...
        if (thread_id == 0)
        {
            ...
            if (m_copy_to_restore == nullptr)
            {
                fprintf(
                    stderr,
                    "[ThreadAffinity::ctor] ERROR: Not enough space to "
                    "allocate a cpu_set_t\n");
                exit(EXIT_FAILURE);
            }
            ...
            if (rc != 0)
            {
                fprintf(
                    stderr,
                    "[ThreadAffinity::ctor] ERROR: call to sched_getaffinity "
        ...
        if (rc != 0)
        {
            fprintf(
                stderr,
                "[ThreadAffinity::ctor] ERROR: call to sched_setaffinity failed, "
                "errno: %d, strerror: %s\n",
                errno,
                strerror(errno));
            exit(EXIT_FAILURE);
...
ThreadAffinity::~ThreadAffinity()
{
    if (is_enabled() && m_copy_to_restore != nullptr)
    {
        ...
        if (rc != 0)
        {
            fprintf(
                stderr,
                "[ThreadAffinity::dtor] ERROR: Cannot restore the cpu_set_t, "
                "errno: %d, strerror: %s\n",
                errno,
                strerror(errno));
            exit(EXIT_FAILURE);
        }
        free(m_copy_to_restore);
        m_copy_to_restore = nullptr;
    }
}
...
RegisterThread::RegisterThread(::teseo::Teseo * teseo)
...
RegisterThread::RegisterThread(const RegisterThread & rt)
    : m_encoded_addr(reinterpret_cast<uint64_t>(rt.teseo()))
{
    COUT_DEBUG(
        "copy ctor, omp thread: " << omp_get_thread_num() << ", thread_id: " << pthread_self()
                                  << ", enabled: " << is_enabled());
...
OpenMP::OpenMP(TeseoDriver * driver)
    ...
    , m_transaction(reinterpret_cast<teseo::Teseo *>(driver->handle_impl())
                        ->start_transaction(driver->has_read_only_transactions()))
...
OpenMP::OpenMP(const OpenMP & copy)
    : m_thread_affinity(copy.m_thread_affinity.is_enabled() && omp_get_thread_num() != 0)
    , /* do not re-init the master thread */
