...
#define COUT_DEBUG_FORCE(msg)                                                                                   \
    {                                                                                                           \
        ...
        std::cout << "[AdjacencyList::" << __FUNCTION__ << "] [" << concurrency::get_thread_id() << "] " << msg \
                  << std::endl;                                                                                 \
    }
#if defined(DEBUG)
...
AdjacencyList::AdjacencyList(bool is_directed, bool is_thread_safe)
    : m_is_directed(is_directed)
    , m_is_thread_safe(is_thread_safe)
{}
...
double AdjacencyList::get_weight(uint64_t source, uint64_t destination) const
{
    ...
    if (vertex_src == end(m_adjacency_list))
        return NaN;

    auto & outgoing_edges = vertex_src->second.first;
    auto result
        = find_if(begin(outgoing_edges), end(outgoing_edges), [destination](const pair<uint64_t, double> & edge) {
              return edge.first == destination;
          });

    if (result == end(outgoing_edges))
        return NaN;
...
uint64_t AdjacencyList::get_degree(uint64_t vertex_id) const
{
    ...
    else
    {
        ...
        for (const auto & e : vertex->second.first)
        {
            neighbours.insert(e.first);
        }
        for (const auto & e : vertex->second.second)
        {
            neighbours.insert(e.first);
        }
...
const AdjacencyList::EdgeList & AdjacencyList::get_incoming_edges(uint64_t vertex_id) const
{
    ...
    if (it == end(m_adjacency_list))
        ERROR("The searched vertex `" << vertex_id << "' does not exist");
...
void AdjacencyList::for_all_edges(const EdgePair & p, Function f)
{
    if (!is_directed())
    {
        ...
        {
            std::invoke(f, e.first);
        }
    }
    else
    {
        // avoid invoking f on the same edge having the same two endpoints ( a -> b
        // and a <- b )
        std::unordered_set<uint64_t> neighbours;
...
bool AdjacencyList::delete_vertex0(uint64_t vertex_id)
{
    ...
    assert(m_is_directed || vertex_src->second.second.empty()); // if the graph is undirected, then the list of incoming
...
bool AdjacencyList::add_edge(graph::WeightedEdge e)
{
    ...
    if (e.source() == e.destination())
        INVALID_ARGUMENT("Cannot insert an edge with the same source and destination: " << e);
...
bool AdjacencyList::delete_edge0(graph::Edge e)
{
    ...
    if (vertex_dst == end(m_adjacency_list))
        return false;
    ...
    {
        return false; // if there is no outgoing edge from source to destination,
            // there cannot be an incoming edge from destination to source
    }
...
#define CHECK_TIMEOUT                                                                                       \
    if (has_timeout() && (chrono::steady_clock::now() - time_start) > m_timeout)                            \
    {                                                                                                       \
        RAISE_EXCEPTION(                                                                                    \
            TimeoutError,                                                                                   \
...
void AdjacencyList::bfs(uint64_t source_vertex_id, const char * dump2file)
{
    ...
    // init
    unordered_map<uint64_t, int64_t> distances;
    distances[source_vertex_id] = 0;
    common::CircularArray<uint64_t> queue;
    queue.append(source_vertex_id);

    while (!queue.empty())
    {
        CHECK_TIMEOUT

        uint64_t vertex = queue[0];
        queue.pop();
...
void AdjacencyList::pagerank(uint64_t num_iterations, double damping_factor, const char * dump2file)
{
    ...
    for (uint64_t iter = 0; iter < num_iterations; iter++)
    {
        ...
        // step #1, compute the `leakage', the score repartitioned from the sinks to
        // all the other nodes in the graph lines 5 - 10 of the spec v1.0 pp 36
        double dangling_sum = 0;
        for (uint64_t v : sinks)
        {
            dangling_sum += rank[v];
        }
        ...
        for (const auto & p : m_adjacency_list)
        {
            ...
            {
                //                uint64_t source_id = e.first == vertex_id ? e.second :
                //                e.first; // in case the graph is undirected, the
                //                vertex id may be reversed
                uint64_t source_id = e.first;
                uint64_t nout = m_adjacency_list.find(source_id)->second.first.size();
                if (nout > 0)
                {
                    score += rank[source_id] / nout;
                }
            }
...
void AdjacencyList::cdlp(uint64_t max_iterations, const char * dump2file)
{
    ...
    while (iteration <= max_iterations && change)
    {
        ...
        for (auto & v : m_adjacency_list)
        {
            ...
            for (auto & e : v.second.first)
            { // outgoing edges
                histogram[labels[e.first]] += 1;
            }
            for (auto & e : v.second.second)
            { // incoming edges
                histogram[labels[e.first]] += 1;
            }
            ...
            for (auto & h : histogram)
            {
                ...
                else if (h.second == max_count && h.first < label_id)
                { // select the minimum label with the
                    // highest count
                    label_id = h.first;
                }
            }
            //            COUT_DEBUG("label selected: " << label_id << ", count: " <<
            //            max_count);
...
void AdjacencyList::lcc_undirected(unordered_map<uint64_t, double> & result)
{
    ...
    nodes.reserve(m_adjacency_list.size());
    for (const auto & p : m_adjacency_list)
    {
    ...
    for (auto vertex1 : nodes)
    {
        ...
        if (degree >= 2)
        {
            ...
            //            COUT_DEBUG("vertex1: " << vertex1 << ", degree: " << degree
            ...
            for (auto & p : vertex1_neighbours.first)
            {
                ...
                //                COUT_DEBUG("vertex1: " << vertex1 << ", vertex2: " <<
                //                vertex2);
...
void AdjacencyList::sssp(uint64_t source_vertex_id, const char * dump2file)
{
    ...
    auto comparator = [](const elt & e1, const elt & e2) {
        return e1.m_distance > e2.m_distance;
    };
    ...
    { // Dijkstra
        ...
        if (!visited.insert(elt.m_vertex_id).second)
            continue; // we already processed this vertex
