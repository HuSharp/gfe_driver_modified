...
static string libraries_help_screen()
{
    ...
    sort(begin(libs), end(libs), [](const auto & lib1, const auto & lib2) { return lib1.m_name < lib2.m_name; });
...
void Configuration::initialise(int argc, char * argv[])
{
    ...
    options.add_options("Generic")(
        "aging_cooloff",
        "The amount of time to wait idle after the simulation completed in the "
        "Aging2 experiment. The purpose is to measure the memory footprint of "
        "the test library when no updates are being executed",
        value<DurationQuantity>())(
        "aging_memfp",
        "Whether to measure the memory footprint",
        value<bool>()->default_value("false"))(
        "aging_memfp_physical",
        "Whether to consider the virtual or the physical memory in the memory "
        "footprint",
        value<bool>()->default_value("false"))(
        "aging_memfp_report",
        "Whether to log to stdout the memory footprint measurements observed",
        value<bool>()->default_value("false"))(
        "aging_memfp_threshold",
        "Forcedly stop the execution of the aging experiment if the memory "
        "footprint of the whole process is above this threshold",
        value<ComputerQuantity>())(
        "aging_release_memory",
        "Whether to release the memory from the "
        "driver as the experiment proceeds",
        value<bool>()->default_value("true"))(
        "aging_step_size",
        "The step of each recording for the measured progress in the Aging2 "
        "experiment. Valid values are 0.1, 0.25, 0.5 and 1.0",
        value<double>()->default_value("1"))(
        "aging_timeout",
        "Force terminating the aging experiment after the given amount of time "
        "(excl. cool-off time)",
        value<DurationQuantity>())(
        "blacklist",
        "Comma separated list of graph algorithms to "
        "blacklist and do not execute",
        value<string>())(
        "build_frequency",
        "The frequency to build a new snapshot in the aging experiment (default: "
        "disabled)",
        value<DurationQuantity>())(
        "d, database",
        "Store the current configuration value into "
        "the a sqlite3 database at the given location",
        value<string>())(
        "efe",
        "Expansion factor for the edges in the graph",
        value<double>()->default_value(to_string(get_ef_edges())))(
        "efv",
        "Expansion factor for the vertices in the graph",
        value<double>()->default_value(to_string(get_ef_vertices())))(
        "G, graph",
        "The path to the graph to load",
        value<string>())("h, help", "Show this help menu")(
        "latency",
        "Measure the latency of inserts/updates, report the average, "
        "median, std. dev. and 90/95/97/99 percentiles")("l, library", libraries_help_screen(), value<string>())(
        "load",
        "Load the graph into the library in one go")(
        "log",
        "Repeat the log of updates specified in the given file",
        value<string>())(
        "max_weight",
        "The maximum weight that can be assigned when reading "
        "non weighted graphs",
        value<double>()->default_value(to_string(max_weight())))(
        "omp",
        "Maximum number of threads that can be used by OpenMP (0 = do not "
        "change)",
        value<int>()->default_value(to_string(num_threads_omp())))(
        "R, repetitions",
        "The number of repetitions of the same experiment (where applicable)",
        value<uint64_t>()->default_value(to_string(num_repetitions())))(
        "r, readers",
        "The number of client threads to use for the read operations",
        value<int>()->default_value(to_string(num_threads(THREADS_READ))))(
        "seed",
        "Random seed used in various places in the experiments",
        value<uint64_t>()->default_value(to_string(seed())))(
        "t, threads",
        "The number of threads to use for both the read and write operations",
        value<int>()->default_value(to_string(num_threads(THREADS_TOTAL))))(
        "timeout",
        "Set the maximum time for an operation to complete, in seconds",
        value<uint64_t>()->default_value(to_string(get_timeout_graphalytics())))(
        "u, undirected",
        "Is the graph undirected? By default, it's considered directed.")(
        "v, validate",
        "Whether to validate the output results of the Graphalytics algorithms",
        value<string>()->implicit_value("<path>"))(
        "w, writers",
        "The number of client threads to use for the write operations",
        value<int>()->default_value(to_string(num_threads(THREADS_WRITE))))(
        "b, block_size",
        "The block size for Sortledton to use.",
        value<int>()->default_value("1024"))(
        "m, mixed_workload",
        "If set run updates and analytics concurrently.",
        value<bool>()->default_value("false"))(
        "is_timestamped",
    ...
    try
    {
        ...
        if (result["log"].count() > 0)
        {
            ...
            if (!common::filesystem::exists(m_update_log))
            {
                ERROR("Option --log \"" << m_update_log << "\", the file does not exist");
            }

            // verify that the properties from the log file are not also specified via
            // command line
            if (result["efe"].count() > 0)
            {
                ERROR("Cannot specify the option --efe together with the log file");
            }
            if (result["efv"].count() > 0)
            {
                ERROR("Cannot specify the option --efv together with the log file");
            }
            ...
            if (log_properties.find("aging_coeff") == log_properties.end())
            {
                ERROR("The log file `" << m_update_log << "' does not contain the expected property 'aging_coeff'");
            }
            set_coeff_aging(stod(log_properties["aging_coeff"]));
            if (log_properties.find("ef_edges") == log_properties.end())
            {
                ERROR("The log file `" << m_update_log << "' does not contain the expected property 'ef_edges'");
            }
            set_ef_edges(stod(log_properties["ef_edges"]));
            if (log_properties.find("ef_vertices") == log_properties.end())
            {
                ERROR("The log file `" << m_update_log << "' does not contain the expected property 'ef_vertices'");
            }
            set_ef_vertices(stod(log_properties["ef_vertices"]));
            if (log_properties.find("max_weight") != log_properties.end())
            { // otherwise assume the default
                set_max_weight(stod(log_properties["max_weight"]));
            }

            // validate that the graph in the input log file matches the same graph
            ...
            if (it_path_graph != log_properties.end() && result["graph"].count() > 0)
            {
                ...
                auto name_param
                    = common::filesystem::filename(common::filesystem::absolute_path(result["graph"].as<string>()));
                if (name_log != name_param)
                {
                    ERROR(
                        "The log file is based on the graph `"
                        << name_log << "', while the parameter --graph refers to `" << name_param << "'");
                }
            }
        }
        ...
        {
            set_database_path(result["database"].as<string>());
        }
        ...
        else
        {
            ...
            transform(begin(library_name), end(library_name), begin(library_name),
                      ::tolower); // make it lower case
            auto libs = library::implementations();
            auto library_found = find_if(begin(libs), end(libs), [&library_name](const auto & candidate) {
                return library_name == candidate.m_name;
            });
            if (library_found == end(libs))
            {
                ERROR("Library not recognised: `" << result["library"].as<string>() << "'");
            }
        ...
        if (result["validate"].count() > 0)
        {
            ...
            {
                if (!common::filesystem::exists(path_validate_graph))
                {
                    ERROR("Option --validate=\"" << path_validate_graph << "\", the file does not exist.");
                }
        ...
        if (result["blacklist"].count() > 0)
        {
            ...
            {
                // trim
                algorithm.erase(algorithm.begin(), std::find_if(algorithm.begin(), algorithm.end(), [](int ch) {
                                    return !std::isspace(ch);
                                }));
                algorithm.erase(
                    std::find_if(algorithm.rbegin(), algorithm.rend(), [](int ch) { return !std::isspace(ch); }).base(),
                    algorithm.end());
...
::common::Database * Configuration::db()
{
    if (m_database == nullptr && has_database())
    {
        ...
        // random value with no semantic, the aim is to simplify the work of
        // ./automerge.pl in recognising duplicate entries
...
{
    if (value <= 0)
        ERROR("Invalid value for max weight: " << value << ". Expected a positive value");
    m_max_weight = value;
}
...
void Configuration::set_aging_step_size(double value)
{
    ...
    {
        ERROR(
            "Invalid value for the aging step size. It must be in (0, 1]. Value "
            "given: "
            << value);
    }
    ...
    {
        ERROR(
            "Value for the step size currently not supported: "
            << value << ". Expected a value such as 1/(step size) is an integer");
    }
...
uint64_t Configuration::get_num_recordings_per_ops() const
{
    ...
    if (::ceil(1.0 / step_size) != ::floor(1.0 / step_size))
    {
        ERROR(
            "Value for the step size currently not supported: "
            << step_size << ". Expected a value such as 1/(step size) is an integer");
    }
...
{
    if (db() == nullptr)
        ERROR("Path where to store the results not set");
    ...
    if (!get_path_graph().empty())
    {
        params.push_back(P{"graph", get_path_graph()});
    }
    ...
    if (!get_update_log().empty())
    {
        ...
        // version 2: log file, follow the same node degree distribution of the
        // input graph version 3: use #add_edge_v2
