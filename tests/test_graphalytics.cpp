...
const std::string path_example_directed = common::filesystem::directory_executable()
    + "/graphs/ldbc_graphalytics/example-directed"; // without the ext .properties
// at the end
const std::string path_example_undirected
    = common::filesystem::directory_executable() + "/graphs/ldbc_graphalytics/example-undirected"; // without the ext
...
static void load_graph(gfe::library::UpdateInterface * interface, const std::string & path_graphalytics_graph)
{
    ...
    {
        interface->add_vertex(vertex_id);
    }
...
static void validate(
    gfe::library::GraphalyticsInterface * interface,
    const std::string & path_graphalytics_graph,
    int algorithms = /* all */ GA_BFS | GA_PAGERANK | GA_WCC | GA_CDLP | GA_LCC | GA_SSSP,
    bool register_thread = true)
{
    ...
    gfe::reader::GraphalyticsReader reader{
        path_graphalytics_graph + ".properties"}; // to parse the properties in the file

    if (algorithms & GA_BFS)
    {
        string path_reference = path_graphalytics_graph + "-BFS";
        if (!common::filesystem::file_exists(path_reference))
            ERROR("The reference output file `" << path_reference << "' does not exist!");
...
TEST(LLAMARef, GraphalyticsCompareWCCWithStinger)
{
    ...
    for (uint64_t i = 0; i < stream.num_edges(); i++)
    {
        ...
        if (i == 1000)
            llama_ref->build(); // create multiple snapshots
    }
...
{
    auto graph = make_unique<GraphOne>(
        /* directed */ true,
        /* vertex dictionary */ true,
        /* blind writes */ true,
        /* ignore build = */ false,
        /* ref impl ? */ false,
        /* max num vertices */ 32);
...
TEST(LiveGraph, GraphalyticsDirected)
{
    ...
    /**
   * On directed graphs, for LiveGraph we only have an implementation of WCC &
   * SSSP. All the other kernel implementations rely on the knowledge of
   * incoming edges, which we are not keeping atm.
   */
