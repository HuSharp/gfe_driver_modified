...
TEST(LLAMA, RewriteEdgeInWriteStore)
{
    ...
    //    ASSERT_EQ( (int) llama.get_weight(20, 30), 2030 ); // only visible after
    //    build?
    ...
    //    ASSERT_EQ( (int) llama.get_weight(30, 20), 3020 ); // only visible after
    //    build?
...
TEST(LLAMA, OutDegreeCounterOverflow)
{ // This is going to take a while, ZzZ...
    LLAMAClass llama{/* directed = */ false};
    llama.add_vertex(1);
    ...
    for (uint64_t thread_id = 0; thread_id < num_threads; thread_id++)
    {
        threads.emplace_back(
            [num_threads, num_vertices, &llama](uint64_t thread_id) {
    ...
    LOG("Updates (% 101)..."); // this block is not necessary for the bug, just to
