...
CByteArray::CByteArray(size_t capacity)
    : CByteArray(compute_bytes_per_elements(capacity), capacity)
{}

CByteArray::CByteArray(size_t bytes_per_element, size_t capacity)
    ...
    , m_array(nullptr)
{
    ...
    if (bytes_per_element <= 0 || bytes_per_element > 8)
        throw std::invalid_argument(
            std::string("Invalid value for the parameter bytes_per_elements: ") + std::to_string(bytes_per_element));
    m_array = new char[m_bytes_per_element * m_capacity];
}

CByteArray::CByteArray(CByteArray && tmp)
...
uint64_t CByteArray::get_value_at(size_t index) const
{
    union
    {
        uint64_t key;
...
void CByteArray::set_value_at(size_t index, uint64_t value)
{
    union
    {
        uint64_t value;
...
void CByteArray::sort(size_t start, size_t end)
{ // sort in [start, end)
...
unique_ptr<CByteArray> CByteArray::merge(CByteArray ** arrays, size_t arrays_sz)
{
    ...
    {
        assert(capacity == 0); // otherwise when updating the capacity this value
            // should have changed
        bytes_per_element = 1;
    }
 ...
 *                                                                           *
 *****************************************************************************/
CByteReference::CByteReference(CByteArray * array, size_t index)
    : m_array(array)
    , m_index(index)
{}

CByteReference::operator uint64_t() const
{
    return m_array->get_value_at(m_index);
}

CByteReference & CByteReference::operator=(uint64_t value)
{
...
CByteIterator::CByteIterator(CByteArray * array, size_t position)
    : m_array(array)
    , m_offset(position)
{}
...
CByteIterator & CByteIterator::operator++()
{ // x++
    m_offset++;
    return *this;
}

CByteIterator CByteIterator::operator++(int)
{ // ++x
    return CByteIterator(m_array, m_offset++);
}

CByteIterator & CByteIterator::operator+=(ptrdiff_t d)
{ // x += d
    m_offset += d;
    return *this;
}

CByteIterator CByteIterator::operator+(std::ptrdiff_t d)
{ // x + d
...
ptrdiff_t CByteIterator::operator+(const CByteIterator & it) const
{ // d = x + y
