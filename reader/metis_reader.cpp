...
#define COUT_DEBUG_FORCE(msg)                                                      \
    {                                                                              \
        std::cout << "[MetisReader::" << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
...
MetisReader::MetisReader(const std::string & path)
    : m_handle(init_fstream(path))
    , m_random_generator(configuration().seed() + 12908478)
{
    ...
    smatch matches;
    if (!regex_match(m_current_line, matches, pattern))
    ...
    //    m_num_edges = stoull(matches[2]); // ignore this field, we're not going
    //    to validate the number of edges parsed

    if (!matches[3].matched)
    { // optional field, according to the manual, it
        // means that all weights not explicitly provided
        // but are implicitly set to 1
    ...
    { // the format is explicitly present. It consists of three digits in
        // {0, 1}
        ...
        { // number of vertex weights to skip
            ...
            m_num_vertex_weights =
                /* vertex size */ (format.c_str()[0] == '1') + ncon;
        }
        else
        {
...
bool MetisReader::fetch_next_line()
{
    ...
    do
    {
        ...
        if (comment)
        {
            COUT_DEBUG("[" << m_lineno << "] line: `" << m_current_line << "' is a comment, skipped");
        }
#endif
    ...
    while (isspace(m_current_line_read_value_ptr[0]))
        m_current_line_read_value_ptr++;
...
std::pair<bool, uint64_t> MetisReader::read_value()
{
    std::pair<bool, uint64_t> result{
        /* were we able to read a value ? */ false,
        /* the value read */ 0};

    if (m_current_line_read_value_ptr != nullptr
        && (m_current_line_read_value_ptr[0] >= '0' && m_current_line_read_value_ptr[0] <= '9'))
    {
        char * next = nullptr;
        result.first = true;
...
bool MetisReader::read(graph::WeightedEdge & e)
{
    ...
    if (!dest_vertex.first)
    { // move to the next line that is not a comment
        if (!fetch_next_line())
            return false; // side effect, it updates the value of m_current_line
        ...
        if (m_edge_vertex1 > m_num_vertices)
        {
            ...
            ERROR(
                "line no: " << m_lineno << ", line: `" << m_current_line
                            << "', constraint not respected: the number of "
                               "vertices in the header ("
                            << m_num_vertices
                            << ") is less than the number of vertices read: " << (m_edge_vertex1 - 1));
        }
        ...
        for (int i = 0; i < m_num_vertex_weights; i++)
        {
            ...
            {
                ERROR(
                    "line no: " << m_lineno << ", vertex: " << m_edge_vertex1 << ", cannot skip the first "
                                << m_num_vertex_weights << " vertex weight. Stuck at: " << i << "/"
                                << m_num_vertex_weights);
            }
        }
    ...
    else if (m_has_edge_weight)
    {
        ...
        {
            ERROR(
                "line no: " << m_lineno << ", missing weight for the edge " << m_edge_vertex1 << " -> "
                            << m_edge_vertex2 << ", line: " << m_current_line);
        }
        else if (info_weight.second > (uint64_t)std::numeric_limits<int32_t>::max())
        {
            ERROR(
                "line no: " << m_lineno << ", the weight for the edge " << m_edge_vertex1 << " -> " << m_edge_vertex2
                            << " "
                               "is too big: "
                            << info_weight.second << ", line: " << m_current_line);
        }
        m_edge_weight = info_weight.second;
    }
    else
    { // assign an edge weight randomly
