...
#define COUT_DEBUG_FORCE(msg)                                                             \
    {                                                                                     \
        std::cout << "[GraphalyticsReader::" << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
...
// Check whether the given line is a comment or empty, that is, it starts with a
// sharp symbol # or contains no symbols
...
// text format, i.e. the original format mandated by the Graphalytics
// specification
...
public:
    GraphalyticsPlainReader(const string & path_vertex_file, const string & path_edge_file, bool is_weighted)
    ...
    bool read_edge(uint64_t & source, uint64_t & destination, double & weight) override
    {
        ...
        do
        {
            ...
            if (skip)
            {
                COUT_DEBUG("line: `" << current_line << "' is a comment or an empty line, skipped");
            }
#endif
        } while (skip && m_handle_edge_file.good());
        if (skip)
            return false; // ended with a comment
        ...
        if (!is_number(current))
            ERROR("line: `" << current_line << "', cannot read the source vertex");
        ...
        current = next;
        if (!is_number(current))
            ERROR("line: `" << current_line << "', cannot read the destination vertex");
    ...
    bool read_vertex(uint64_t & out_vertex) override
    {
        ...
        while (isspace(current[0]))
            current++;
        if (!is_number(current))
            ERROR("line: `" << current_line << "', cannot read the vertex id");
...
public:
    GraphalyticsZlibDecompressInput(const string & path_vertex_file)
    {
        ...
        if (!m_handle.good())
        {
            ERROR("Cannot open the input file: " << path_vertex_file);
        }
    ...
    bool read(T * item)
    {
        if (m_output_pos >= m_output_sz)
        {
            ...
            if (rc != Z_OK && rc != Z_STREAM_END)
                ERROR("Cannot decompress the input stream: " << m_stream.msg << " (rc: " << rc << ")");
...
public:
    GraphalyticsZlibReader2(const string & path_vertex_file, const string & path_edge_file)
        : m_vertices(path_vertex_file)
        , m_edges(path_edge_file)
    {
    ...
    // read one vertex at the time from the vertex file
    bool read_vertex(uint64_t & out_vertex) override { return m_vertices.read(&out_vertex); }
};
...
public:
    GraphalyticsZlibReader3(const string & path_vertex_file, const string & path_edge_file)
        : m_vertices(path_vertex_file)
        , m_edges(path_edge_file)
    {
...
GraphalyticsReader::GraphalyticsReader(const std::string & path_properties)
    : m_random_generator(configuration().seed() + 12908478)
{
    if (!common::filesystem::file_exists(path_properties))
        ERROR("The given file does not exist: " << path_properties);
    ...
    while (handle.good())
    {
        ...
        { // side effect => populate matches
            ...
            if (name.empty())
            {
                ...
                if (pos == string::npos)
                    ERROR(
                        "Cannot parse the name of the graph (expected to find "
                        "graph.<GRAPH_NAME>.<PROPERTY_NAME>), key=graph."
                        << key);
            ...
            {
                LOG("[GraphalyticsReader] Warning, line skipped `"
                    << line << "', graph name does not match what expected: " << name);
                continue;
            }
            ...
            {
                if (value.empty())
                    ERROR("Empty path specified for the key " << key << ", line: " << line);
                if (value[0] != '/')
                {
            ...
            else if (key == "directed")
            {
                ...
                {
                    ERROR(
                        "Cannot determine whether the graph is directed or not. The "
                        "property value is `"
                        << value << "'");
                }
            }
    ...
    if (m_properties.find("vertex-file") == m_properties.end())
        ERROR("The property `vertex-file' is not set in the property file");
    if (m_properties.find("edge-file") == m_properties.end())
        ERROR("The property `edge-file' is not set in the property file");
    COUT_DEBUG(
        "vertex-file: " << get_path_vertex_list() << ", edge-file: " << get_path_edge_list()
...
string GraphalyticsReader::get_path_vertex_list() const
{
    ...
    if (item == m_properties.end())
        ERROR("The property `vertex-file' is not set");
    return item->second;
}
...
string GraphalyticsReader::get_path_edge_list() const
{
    ...
    if (item == m_properties.end())
        ERROR("The property `edge-file' is not set");
    return item->second;
}
...
bool GraphalyticsReader::read_edge(graph::WeightedEdge & edge)
{
    ...
    else
    {
        ...
        if (!is_weighted())
        {
            ...
            if (m_last_weight == 0.0)
                m_last_weight = configuration().max_weight(); // in (0, max_weight]
        }
