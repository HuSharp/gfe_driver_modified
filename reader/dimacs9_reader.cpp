...
#define COUT_DEBUG_FORCE(msg)                                                        \
    {                                                                                \
        std::cout << "[Dimacs9Reader::" << __FUNCTION__ << "] " << msg << std::endl; \
    }
#if defined(DEBUG)
 ...
 *                                                                           *
 *****************************************************************************/
Dimacs9Reader::Dimacs9Reader(const std::string & path)
...
bool Dimacs9Reader::parse_line(char *& line)
{
    ...
    {
        return false;
    } // this does not represent an edge
    do
    {
        line++;
    } while (isspace(line[0])); // ignore the following spaces
    return true;
}
...
bool Dimacs9Reader::read(graph::WeightedEdge & edge)
{
    ...
    do
    {
        ...
        if (skip)
        {
            COUT_DEBUG("line `" << m_buffer << "' skipped");
        }
#endif
    ...
    if (!is_number(current))
        ERROR("line: `" << m_buffer << "', cannot read the source vertex");
    ...
    current = next;
    if (!is_number(current))
        ERROR("line: `" << m_buffer << "', cannot read the destination vertex");
    ...
    current = next;
    if (!is_number(current))
        ERROR("line: `" << m_buffer << "', cannot read the weight");
