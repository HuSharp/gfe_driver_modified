...
static void run_standalone(int argc, char * argv[])
{
    ...
    if (configuration().is_load())
    {
        ...
        if (impl_load.get() == nullptr)
        {
            ERROR("The library `" << configuration().get_library_name() << "' does not support loading");
        }
        auto impl_rndvtx = dynamic_pointer_cast<library::RandomVertexInterface>(impl);
        if (impl_rndvtx.get() == nullptr)
        {
            ERROR("The library `" << configuration().get_library_name() << "' does not allow to fetch a random vertex");
        }
    ...
    else
    {
        ...
        if (impl_upd.get() == nullptr)
        {
            ERROR("The library `" << configuration().get_library_name() << "' does not support updates");
        }
        ...
        if (configuration().get_update_log().empty())
        {
            ...
            if (stream->num_edges() > 0)
                random_vertex = stream->get(0).m_source;

            LOG("[driver] Number of concurrent threads: " << configuration().num_threads(THREADS_WRITE));

            if (configuration().measure_latency())
                ERROR("[driver] InsertOnly, support for latency measurements removed");
        ...
        else
        {
            ...
            else
            {
                ...
                if (configuration().validate_inserts() && impl_upd->can_be_validated())
                {
                    LOG("[driver] Validation of updates requested, loading the original "
                        "graph from: "
                        << path_graph);
                    auto stream = make_shared<graph::WeightedEdgeStream>(configuration().get_path_graph());
                    num_validation_errors = validate_updates(impl_upd, stream);
    ...
    if (configuration().num_repetitions() > 0 && !configuration().is_mixed_workload())
    {
        ...
        {
            LOG("[driver] OpenMP, number of threads for the Graphalytics suite: "
                << configuration().num_threads(ThreadsType::THREADS_READ));
